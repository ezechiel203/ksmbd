...
smb2_get_name(const char *src, const int maxlen, struct nls_table *local_nls)
{
	...
	if (*name == '\\') {
		pr_err_ratelimited("not allow directory name included leading slash\n");
		kfree(name);
		return ERR_PTR(-EINVAL);
	}
...
int smb2_open(struct ksmbd_work *work)
{
	...
	if (req->NameLength) {
		...
		/*
		 * Validate the filename at the UTF-16LE code-point level
		 * BEFORE converting to the local charset.  This avoids
		 * false rejections: converting unknown code points to '?'
		 * and then failing the ASCII wildcard check.  Only Windows-
		 * reserved characters are rejected here; Unicode combining
		 * characters, surrogate pairs, fullwidth forms, etc. are
		 * explicitly allowed per MS-SMB2 §2.2.13.
		 */
		rc = ksmbd_validate_utf16_filename(
			(__le16 *)((char *)req + le16_to_cpu(req->NameOffset)),
	...
	if (!S_ISDIR(file_inode(filp)->i_mode) && open_flags & O_TRUNC &&
		...
		need_truncate = 1;
		/* Overwriting a file modifies the parent directory */
	...
	/*
	 * Break parent directory leases when a new file is created.
	 * smb_send_parent_lease_break_noti() handles the case when the
	 * creating client also has a lease with a parent key, but we
	 * still need to break dir leases for creates without lease context.
	 */
	if (file_info == FILE_CREATED && !lc)
		smb_break_parent_dir_lease(fp);
...
continue_create: ;
	...
	if (dh_info.type == DURABLE_REQ_V2 || dh_info.type == DURABLE_REQ) {
		if (dh_info.type == DURABLE_REQ_V2 &&
		    (dh_info.persistent & SMB2_DHANDLE_FLAG_PERSISTENT) &&
	...
	/*
	 * MS-SMB2 §2.2.13.2.13 / §2.2.13.2.18  APP_INSTANCE_ID /
	 * APP_INSTANCE_VERSION create contexts.
	 *
	 * Process VERSION before ID so that version fields are populated
	 * on @fp before the ID handler calls ksmbd_close_prev_app_instance().
	 * Both contexts are GUID-tagged (16-byte name).
	 */
	if (req->CreateContextsOffset) {
		struct create_context *app_ver_ctx;
		struct create_context *app_id_ctx;

		/* --- APP_INSTANCE_VERSION --- */
		app_ver_ctx = smb2_find_context_vals(req,
				SMB2_CREATE_APP_INSTANCE_VERSION, 16);
		if (IS_ERR(app_ver_ctx)) {
			rc = PTR_ERR(app_ver_ctx);
			goto err_out1;
		}
		if (app_ver_ctx) {
			/*
			 * APP_INSTANCE_VERSION data layout (MS-SMB2 §2.2.13.2.18):
			 *   [0..1]  StructureSize (must be 24)
			 *   [2..3]  Padding
			 *   [4..7]  Reserved
			 *   [8..15] AppInstanceVersionHigh (__le64)
			 *   [16..23] AppInstanceVersionLow (__le64)
			 *
			 * DataOffset points past the 16-byte tag name; the data
			 * blob starts at cc + DataOffset.
			 */
			u16 ver_data_off = le16_to_cpu(app_ver_ctx->DataOffset);
			u32 ver_data_len = le32_to_cpu(app_ver_ctx->DataLength);
			const u8 *vdata;

			if (ver_data_len >= 24) {
				vdata = (const u8 *)app_ver_ctx + ver_data_off;
				fp->app_instance_version =
					get_unaligned_le64(vdata + 8);
				fp->app_instance_version_low =
					get_unaligned_le64(vdata + 16);
				fp->has_app_instance_version = true;
				ksmbd_debug(SMB,
					    "APP_INSTANCE_VERSION: high=%llu low=%llu\n",
					    fp->app_instance_version,
					    fp->app_instance_version_low);
			}
		}
		...
		/* --- APP_INSTANCE_ID --- */
		app_id_ctx = smb2_find_context_vals(req,
				SMB2_CREATE_APP_INSTANCE_ID, 16);
		if (IS_ERR(app_id_ctx)) {
			rc = PTR_ERR(app_id_ctx);
			goto err_out1;
		}
		if (app_id_ctx) {
			/*
			 * APP_INSTANCE_ID data layout (MS-SMB2 §2.2.13.2.13):
			 *   [0..1]  StructureSize (must be 20)
			 *   [2..3]  Reserved
			 *   [4..19] AppInstanceId (16-byte GUID)
			 */
			u16 id_data_off = le16_to_cpu(app_id_ctx->DataOffset);
			u32 id_data_len = le32_to_cpu(app_id_ctx->DataLength);
			const u8 *idata;
			static const u8 zero_guid[16] = {};

			if (id_data_len >= 20) {
				idata = (const u8 *)app_id_ctx + id_data_off;
				/* GUID starts at offset 4 within the data */
				if (!memcmp(idata + 4, zero_guid, 16)) {
					ksmbd_debug(SMB,
						    "APP_INSTANCE_ID: zero GUID, ignoring\n");
				} else {
					...
					ksmbd_debug(SMB,
						    "APP_INSTANCE_ID: set on fid=%llu\n",
						    fp->volatile_id);

					rc = ksmbd_close_prev_app_instance(fp);
					if (rc) {
						/*
						 * Existing instance has equal-or-newer
						 * version; reject this open.
						 */
						rsp->hdr.Status =
				       ...
				       struct ksmbd_kstat *ksmbd_kstat)
{
	...
	if (check_add_overflow(readdir_info_level_struct_sz(info_level),
			       conv_len, &struct_sz)) {
		rc = -EOVERFLOW;
		goto free_conv_name;
	}
				   ...
				   int info_level)
{
	...
	if (check_add_overflow(struct_sz, conv_len, &next_entry_offset)) {
		d_info->out_buf_len = 0;
		return -EOVERFLOW;
	}
	next_entry_offset = ALIGN(next_entry_offset, KSMBD_DIR_INFO_ALIGNMENT);
...
int smb2_query_dir(struct ksmbd_work *work)
{
	...
	if (!work->tcon || !work->tcon->share_conf) {
		pr_err("smb2_query_dir: missing tcon or share_conf\n");
		return -EINVAL;
	}
			       ...
			       struct ksmbd_file *fp, void *rsp_org)
{
	...
	basic_info->Attributes = fp->f_ci ? fp->f_ci->m_fattr : 0;
	basic_info->Pad1 = 0;
	rsp->OutputBufferLength =
			     ...
			     void *rsp_org)
{
	...
	file_info->Attributes = fp->f_ci ? fp->f_ci->m_fattr : 0;
	file_info->Pad1 = 0;
	if (ksmbd_stream_fd(fp) == false) {
				       ...
				       struct ksmbd_file *fp, void *rsp_org)
{
	...
	file_info->FileAttributes = fp->f_ci ? fp->f_ci->m_fattr : 0;
				...
				struct ksmbd_file *fp, void *rsp_org)
{
	...
	time = ksmbd_UnixTimeToNT(stat.ctime);
	file_info->ChangeTime = cpu_to_le64(time);
	file_info->DosAttributes = fp->f_ci ? fp->f_ci->m_fattr : 0;
				    ...
				    struct smb2_query_info_rsp *rsp)
{
	...
	case FS_CONTROL_INFORMATION:
	{
		...
		/*
		 * Query real VFS quotas if the filesystem supports them.
		 * Fall back to unlimited defaults otherwise.
		 */
		if (sb->s_qcop && sb->s_qcop->get_dqblk) {
			struct kqid qid = make_kqid_uid(current_fsuid());
			struct qc_dqblk dqblk;
			int qrc;

			memset(&dqblk, 0, sizeof(dqblk));
			qrc = sb->s_qcop->get_dqblk(sb, qid, &dqblk);
			if (!qrc) {
				info->DefaultQuotaThreshold =
					cpu_to_le64(dqblk.d_spc_softlimit);
				info->DefaultQuotaLimit =
					cpu_to_le64(dqblk.d_spc_hardlimit);
			} else {
				/* Quota query failed, return unlimited */
				info->DefaultQuotaThreshold =
			     ...
			     struct smb2_query_info_rsp *rsp)
{
	...
	/*
	 * MS-SMB2 §3.3.5.20.1: LABEL_SECURITY_INFORMATION (0x10)
		...
		if (label_len > 0 &&
			...
			if (secdesclen + sizeof(struct smb_acl) <= resp_len) {
				struct smb_acl *empty_sacl =
					(struct smb_acl *)((char *)pntsd + secdesclen);

				empty_sacl->revision = cpu_to_le16(2);
				empty_sacl->size = cpu_to_le16(sizeof(struct smb_acl));
...
int smb2_close(struct ksmbd_work *work)
{
	...
	if (req->Flags == SMB2_CLOSE_FLAG_POSTQUERY_ATTRIB) {
		...
		rsp->Attributes = fp->f_ci ? fp->f_ci->m_fattr : 0;
		rsp->CreationTime = cpu_to_le64(fp->create_time);
		time = ksmbd_UnixTimeToNT(stat.atime);
		rsp->LastAccessTime = cpu_to_le64(time);
		time = ksmbd_UnixTimeToNT(stat.mtime);
		       ...
		       struct nls_table *local_nls)
{
	...
	smb_break_all_levII_oplock(work, fp, 0);
	{
		...
		src_parent = dget(fp->filp->f_path.dentry->d_parent);
		rc = ksmbd_vfs_fp_rename(work, fp, new_name);
		if (!rc) {
			smb_break_dir_lease(src_parent);
			smb_break_parent_dir_lease(fp);
		}
		dput(src_parent);
	}
			       ...
			       struct ksmbd_share_config *share)
{
	...
	if (test_share_config_flag(share, KSMBD_SHARE_FLAG_STORE_DOS_ATTRS) &&
		...
		da.attr = le32_to_cpu(fp->f_ci ? fp->f_ci->m_fattr : 0);
		da.flags = XATTR_DOSINFO_ATTRIB | XATTR_DOSINFO_CREATE_TIME |
			XATTR_DOSINFO_ITIME;
			      ...
			      struct ksmbd_share_config *share)
{
	...
	case FILE_LINK_INFORMATION:
	{
		...
		ret = smb2_create_link(work, work->tcon->share_conf,
	...
	{
		/*
		 * Accept short name SET requests silently. If the backing
		 * filesystem supports it (e.g. ntfs3 via system.ntfs_dos_name
		 * xattr), attempt to set; otherwise succeed as a no-op since
		 * Windows clients rarely set short names explicitly.
		 */
		struct smb2_file_alt_name_info *alt_info;
		char *short_name_utf8;
		int rc_xattr;

		if (buf_len < sizeof(struct smb2_file_alt_name_info))
			return -EMSGSIZE;

		alt_info = (struct smb2_file_alt_name_info *)buffer;
		if (le32_to_cpu(alt_info->FileNameLength) == 0)
			return 0;

		short_name_utf8 = smb_strndup_from_utf16(alt_info->FileName,
					le32_to_cpu(alt_info->FileNameLength),
					true, work->conn->local_nls);
		if (IS_ERR(short_name_utf8))
			return 0; /* fail silently */

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
		rc_xattr = ksmbd_vfs_setxattr(mnt_idmap(fp->filp->f_path.mnt),
#else
		rc_xattr = ksmbd_vfs_setxattr(mnt_user_ns(fp->filp->f_path.mnt),
#endif
					       &fp->filp->f_path,
					       "system.ntfs_dos_name",
					       short_name_utf8,
					       strlen(short_name_utf8), 0,
					       true);
		kfree(short_name_utf8);
		/* Ignore errors — most filesystems don't support this xattr */
		if (rc_xattr)
			ksmbd_debug(SMB,
				    "Failed to set short name xattr (rc=%d), ignoring\n",
				    rc_xattr);
		return 0;
	}
	}
...
int smb2_set_info(struct ksmbd_work *work)
{
	...
	case SMB2_O_INFO_FILESYSTEM:
	{
		int fs_info_class = req->FileInfoClass;

		ksmbd_debug(SMB, "GOT SMB2_O_INFO_FILESYSTEM, class %d\n",
			    fs_info_class);
		if (fs_info_class == FS_LABEL_INFORMATION) {
			/*
			 * MS-FSCC 2.4.34 FILE_FS_LABEL_INFORMATION SET:
			 *   VolumeLabelLength (__le32) + VolumeLabel (UTF-16LE)
			 */
			char *buffer;
			__le32 *label_len_ptr;
			...
			if (le32_to_cpu(req->BufferLength) < sizeof(__le32)) {
				rc = -EMSGSIZE;
				goto err_out;
			}
			label_len_ptr = (__le32 *)buffer;
			label_len = le32_to_cpu(*label_len_ptr);
			if (label_len <= 0 || label_len > PATH_MAX) {
				rc = -EINVAL;
				goto err_out;
			}

			label_utf8 = smb_strndup_from_utf16(
					buffer + sizeof(__le32),
					label_len, true,
					work->conn->local_nls);
			if (IS_ERR(label_utf8)) {
				rc = PTR_ERR(label_utf8);
				goto err_out;
			}

			/* Copy into fixed-size buffer for ioctl */
			memset(label_buf, 0, sizeof(label_buf));
			strscpy(label_buf, label_utf8, sizeof(label_buf));
			kfree(label_utf8);

			/* Open share root to get a file handle for ioctl */
			rc = kern_path(work->tcon->share_conf->path,
				       LOOKUP_NO_SYMLINKS, &share_path);
			if (rc) {
				rc = -EIO;
				goto err_out;
			}

			filp = dentry_open(&share_path, O_RDONLY,
					   current_cred());
			path_put(&share_path);
			if (IS_ERR(filp)) {
				ksmbd_debug(SMB,
					    "Failed to open share root for label set\n");
				rc = -EOPNOTSUPP;
				goto err_out;
			}

			if (filp->f_op && filp->f_op->unlocked_ioctl) {
				rc = filp->f_op->unlocked_ioctl(filp,
						FS_IOC_SETFSLABEL,
						(unsigned long)label_buf);
			} else {
				rc = -EOPNOTSUPP;
			}
			fput(filp);

			if (rc == -ENOTTY || rc == -EOPNOTSUPP) {
				ksmbd_debug(SMB,
					    "FS does not support setting label\n");
				rc = -EOPNOTSUPP;
			}
		} else {
			rc = -EOPNOTSUPP;
...
skip:
		if (smb_lock->flags & SMB2_LOCKFLAG_UNLOCK) {
			...
			if (rc == FILE_LOCK_DEFERRED) {
				...
				rc = ksmbd_vfs_posix_lock_wait(flock);
	...
	if (fp->f_ci && atomic_read(&fp->f_ci->op_count) > 1)
		smb_break_all_oplock(work, fp);
...
dup_ext_out:
	...
	case FSCTL_GET_COMPRESSION:
	{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 13, 0)
		{
			...
			err = vfs_fileattr_get(file_dentry(fp->filp), &fa);
			if (!err && fa.flags_valid &&
			    (fa.flags & FS_COMPR_FL))
				*(__le16 *)&rsp->Buffer[0] =
					cpu_to_le16(0x0002); /* LZNT1 */
			else
				*(__le16 *)&rsp->Buffer[0] =
					cpu_to_le16(0x0000); /* NONE */
		}
#else
		/* Kernel too old for fileattr API, report uncompressed */
		*(__le16 *)&rsp->Buffer[0] = cpu_to_le16(0x0000);
#endif
		ksmbd_fd_put(work, fp);
		nbytes = sizeof(__le16);
		break;
	}
	case FSCTL_SET_COMPRESSION:
	{
		...
		comp_state = *(__le16 *)buffer;

		/* Validate CompressionState value */
		if (le16_to_cpu(comp_state) > 0x0002) {
			rsp->hdr.Status = STATUS_INVALID_PARAMETER;
			ret = -EINVAL;
			goto out;
		}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 13, 0)
		{
			...
			if (le16_to_cpu(comp_state) == 0x0000)
				fa.flags &= ~FS_COMPR_FL;
			else
				fa.flags |= FS_COMPR_FL;
			...
			err = vfs_fileattr_set(
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
				file_mnt_idmap(fp->filp),
#else
				file_mnt_user_ns(fp->filp),
#endif
				file_dentry(fp->filp), &fa);
			if (err) {
				ksmbd_fd_put(work, fp);
				/*
				 * Filesystem does not support compression
				 * attributes.  Return success per Windows
				 * behavior on non-compressed volumes.
				 */
				break;
			}
		}
...
int smb3_check_sign_req(struct ksmbd_work *work)
{
	...
	if (!signing_key) {
		pr_err_ratelimited("SMB3 signing key is not generated\n");
		return 0;
	}
