...
/* IPC-01: Increased from 2s to 10s to avoid false "daemon dead" on slow LDAP */
#define IPC_WAIT_TIMEOUT	(10 * HZ)
...
static int handle_witness_unregister_event(struct sk_buff *skb,
					   struct genl_info *info);
static int handle_share_config_flush_event(struct sk_buff *skb,
					   struct genl_info *info);
...
static struct genl_ops ksmbd_genl_ops[] = {
	...
	{
		.cmd	= KSMBD_EVENT_SHARE_CONFIG_FLUSH,
		.doit	= handle_share_config_flush_event,
		.flags	= GENL_ADMIN_PERM,
	},
};
...
static struct genl_family ksmbd_genl_family = {
	...
	.resv_start_op	= KSMBD_EVENT_SHARE_CONFIG_FLUSH + 1,
#endif
};
...
static int handle_response(int type, void *payload, size_t sz)
{
	...
	if (sz < sizeof(unsigned int))
		return -EINVAL;
	if (sz > KSMBD_IPC_MAX_PAYLOAD)
		return -EINVAL;

	handle = *(unsigned int *)payload;
	if (type == KSMBD_EVENT_RPC_RESPONSE) {
		struct ksmbd_rpc_command *resp = payload;

		if (sz < sizeof(struct ksmbd_rpc_command))
			return -EINVAL;
		handle = resp->xid;
	}
...
static void ipc_msg_table_mark_stale(void)
{
	struct ipc_msg_table_entry *entry;
	...
	hash_for_each_safe(ipc_msg_table, bkt, tmp, entry, ipc_table_hlist) {
		entry->stale = true;
		wake_up_interruptible(&entry->wait);
	}
...
static int ipc_server_config_on_startup(struct ksmbd_startup_request *req)
{
	...
	/*
	 * NETLINK-01: Validate ifc_list_sz against the documented 4KB limit
	 * to prevent OOB kernel memory reads when a malformed daemon sends an
	 * inflated ifc_list_sz in the startup request payload.
	 */
	if (req->ifc_list_sz > KSMBD_REQ_MAX_IFC_LIST_SZ) {
		pr_err("ksmbd: invalid ifc_list_sz %u (max %u)\n",
		       req->ifc_list_sz, KSMBD_REQ_MAX_IFC_LIST_SZ);
		return -EINVAL;
	}
...
/*
 * handle_startup_event - Process daemon startup/reconnect via netlink.
 *
 * This is the entry point for live reconfiguration.  When the daemon
 * reconnects (ksmbd_tools_pid is already set but the old daemon is
 * unreachable), we accept the new PID so subsequent IPC requests are
 * routed to the replacement daemon.  On first connect we apply the
 * full server configuration from the startup request and kick off
 * transport initialization.  See server.h for the complete state
 * machine description.
 */
static int handle_startup_event(struct sk_buff *skb, struct genl_info *info)
{
...

	{
		...
		if (ksmbd_tools_pid) {
			...
			ipc_msg_table_mark_stale();
			ksmbd_share_configs_flush();
			is_reconnect = true;
		}

		/* Always update server config from new daemon's startup request */
...
static int handle_generic_event(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (type > KSMBD_EVENT_MAX) {
		WARN(1, "ksmbd: IPC event type %d exceeds max %d\n",
		     type, KSMBD_EVENT_MAX);
		return -EINVAL;
	}
...
static int ipc_validate_msg(struct ipc_msg_table_entry *entry)
{
	...
	{
		/*
		 * IPC-02: Validate minimum size for login response to prevent
		 * OOB read of uid, gid, hash_sz, and hash fields when a
		 * truncated response is received from a malformed daemon.
		 */
		if (entry->msg_sz < sizeof(struct ksmbd_login_response))
			return -EINVAL;
		msg_sz = sizeof(struct ksmbd_login_response);
		break;
	}
	case KSMBD_EVENT_TREE_CONNECT_REQUEST:
	{
		if (entry->msg_sz < sizeof(struct ksmbd_tree_connect_response))
			return -EINVAL;
		msg_sz = sizeof(struct ksmbd_tree_connect_response);
		break;
	}
	...
	default:
		/*
		 * IPC-02: Reject unknown/unvalidated IPC response types.
		 * All response types dispatched through ipc_msg_send_request
		 * should have explicit size validation above.
		 */
		return -EINVAL;
	}
...
static void *ipc_msg_send_request(struct ksmbd_ipc_msg *msg, unsigned int handle)
{
	...
	/*
	 * TC-16: Heap-allocate the table entry to prevent a use-after-free.
	 * With a stack-allocated entry, after timeout the stack frame was freed
	 * while a concurrent handle_response() thread (holding down_read) could
	 * still write entry->response — a classic UAF on stack memory.
	 * Heap allocation ensures the memory lives until we explicitly free it
	 * after hash_del under write lock, when all readers are excluded.
	 */
...
struct ksmbd_rpc_command *ksmbd_rpc_open(struct ksmbd_session *sess, int handle)
{
	...
	resp = ipc_msg_send_request(msg, req->xid);
	ipc_msg_handle_free(req->xid);
	ipc_msg_free(msg);
	return resp;
}
...
struct ksmbd_rpc_command *ksmbd_rpc_query(struct ksmbd_session *sess, int handle)
{
	...
	req->flags = ksmbd_session_rpc_method(sess, handle);
	req->flags |= rpc_context_flags(sess);
	req->flags |= KSMBD_RPC_QUERY_METHOD;
	req->payload_sz = 0;
	up_read(&sess->rpc_lock);
