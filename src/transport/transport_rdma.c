...
static atomic_t smbd_active_conn;
...
/*
 * Credit pool low-watermark threshold.  When the number of available
 * receive credits drops below this fraction of recv_credit_max, a
 * rate-limited warning is emitted.  Set to 10% of the default 255.
 */
#define SMB_DIRECT_CREDIT_LOW_WATERMARK_PCT	10
...
struct smb_direct_transport {
	...
	/*
	 * Credit accounting for leak detection.
	 *
	 * total_credits_granted: cumulative credits posted to RDMA HW
	 * total_credits_reclaimed: cumulative credits returned (recv
	 *   completions, both success and flush errors)
	 * recv_posted: number of receive buffers currently posted to HW
	 *   (incremented on post, decremented on any completion)
	 *
	 * Invariant (while connected):
	 *   recv_credits + count_avail_recvmsg == recv_credit_max
	 *   recv_posted >= 0
	 *
	 * On disconnect:
	 *   total_credits_granted == total_credits_reclaimed
	 *   recv_posted == 0
	 */
	atomic_t		recv_posted;
	atomic_t		total_credits_granted;
	atomic_t		total_credits_reclaimed;
};
...
static int smb_direct_credit_audit(struct smb_direct_transport *t);
static void smb_direct_reclaim_credits(struct smb_direct_transport *t);
...
static void smb_direct_disconnect_rdma_work(struct work_struct *work)
{
	...
	if (cmpxchg(&t->status, SMB_DIRECT_CS_CONNECTED,
		    SMB_DIRECT_CS_DISCONNECTING) ==
	    SMB_DIRECT_CS_CONNECTED) {
...
static struct smb_direct_transport *alloc_transport(struct rdma_cm_id *cm_id)
{
	...
	atomic_set(&t->total_credits_granted, 0);
	atomic_set(&t->total_credits_reclaimed, 0);
	...
	/*
	 * J.5: IPv6 address enumeration RCU locking note.
	 *
	 * This code extracts the client address from the RDMA CM route
	 * (cm_id->route.addr.dst_addr), which is a struct sockaddr copied
	 * during connection setup.  This is NOT an iteration of inet6_dev
	 * address lists, so no rcu_read_lock() is required here.
	 *
	 * The IPv6 addr_list iteration that DOES need an RCU lock is in
	 * ksmbd_fsctl.c in fsctl_query_iface_info_ioctl() (around the
	 * list_for_each_entry() call on idev6->addr_list).  That function
	 * uses the non-RCU list_for_each_entry() instead of
	 * list_for_each_entry_rcu(), which is a potential use-after-free
	 * race.  The fix is in that file (owned by Track F/IOCTL), not here.
	 * The duplicate implementation in smb2_ioctl.c already uses
	 * list_for_each_entry_rcu() correctly under rcu_read_lock().
	 */
	/* Extract client IP from RDMA CM and compute a proper hash
	 * so connections don't all land in bucket 0.
	 */
	{
		struct sockaddr *dst = (struct sockaddr *)&cm_id->route.addr.dst_addr;
		...
		if (dst->sa_family == AF_INET6) {
			...
			conn->inet_hash = ipv4_addr_hash(sin->sin_addr.s_addr);
		}
#else
		{
			struct sockaddr_in *sin = (struct sockaddr_in *)dst;
...
static void free_transport(struct smb_direct_transport *t)
{
	...
	/*
	 * After ib_drain_qp all completions have been processed.
	 * Reclaim any outstanding credits that were not properly
	 * returned during flush-error processing, then audit.
	 */
	smb_direct_reclaim_credits(t);
	smb_direct_credit_audit(t);
...
static int smb_direct_check_recvmsg(struct smb_direct_recvmsg *recvmsg)
{
	...
	case SMB_DIRECT_MSG_DATA_TRANSFER: {
		...
		u32 d_offset = le32_to_cpu(req->data_offset);
		u32 d_length = le32_to_cpu(req->data_length);

		/*
		 * Validate data_offset and data_length against the actual
		 * receive buffer size, not just logical structure sizes.
		 * A crafted packet could set data_offset beyond the receive
		 * buffer, causing out-of-bounds access.
		 */
		if (d_offset >= recvmsg->sge.length ||
		    d_length > recvmsg->sge.length ||
		    d_offset + d_length > recvmsg->sge.length) {
		...
		if (d_offset + sizeof(struct smb2_hdr) >
		    sizeof(struct smb_direct_data_transfer) + d_length) {
			pr_err("Invalid data_offset %u in SMB_DIRECT data transfer\n",
			       d_offset);
			return -EINVAL;
		}

		if (d_length && d_offset + sizeof(struct smb2_hdr) <=
		    recvmsg->sge.length) {
			struct smb2_hdr *hdr = (struct smb2_hdr *)
				(recvmsg->packet + d_offset);
			ksmbd_debug(RDMA,
				    "CreditGranted: %u, CreditRequested: %u, DataLength: %u, RemainingDataLength: %u, SMB: %x, Command: %u\n",
				    le16_to_cpu(req->credits_granted),
				    le16_to_cpu(req->credits_requested),
				    req->data_length,
				    req->remaining_data_length,
...
/*
 * smb_direct_credit_audit - verify credit accounting invariants
 *
 * Returns 0 if all invariants hold, negative errno otherwise.
 * Caller must NOT hold receive_credit_lock.
 */
static int smb_direct_credit_audit(struct smb_direct_transport *t)
{
	int recv_creds, avail, posted, total_max;
	int granted, reclaimed, leaked;

	spin_lock(&t->receive_credit_lock);
	recv_creds = t->recv_credits;
	avail = t->count_avail_recvmsg;
	total_max = t->recv_credit_max;
	spin_unlock(&t->receive_credit_lock);

	posted = atomic_read(&t->recv_posted);
	granted = atomic_read(&t->total_credits_granted);
	reclaimed = atomic_read(&t->total_credits_reclaimed);

	/*
	 * Invariant 1: recv_credits + count_avail_recvmsg == recv_credit_max
	 * This must always hold because each receive buffer is either
	 * posted to HW (counted in recv_credits) or available in the
	 * free queue (counted in count_avail_recvmsg).
	 */
	if (recv_creds + avail != total_max) {
		pr_warn_ratelimited(
			"ksmbd: RDMA credit invariant violation: "
			"recv_credits(%d) + avail(%d) != max(%d)\n",
			recv_creds, avail, total_max);
		return -EINVAL;
	}

	/*
	 * Invariant 2: recv_posted must be non-negative.
	 */
	if (posted < 0) {
		pr_warn_ratelimited(
			"ksmbd: RDMA recv_posted underflow: %d\n", posted);
		return -EINVAL;
	}

	/*
	 * Invariant 3: cumulative leak check (only meaningful after
	 * all completions have been processed, e.g. after ib_drain_qp).
	 */
	leaked = granted - reclaimed;
	if (t->status == SMB_DIRECT_CS_DISCONNECTED && leaked != 0) {
		pr_warn_ratelimited(
			"ksmbd: RDMA credit leak detected: "
			"granted(%d) - reclaimed(%d) = %d\n",
			granted, reclaimed, leaked);
		return -EINVAL;
	}
...
/*
 * smb_direct_reclaim_credits - reclaim all outstanding posted credits
 *
 * Called during disconnect/teardown after ib_drain_qp() has ensured all
 * completions have been processed.  Any remaining positive recv_posted
 * represents credits that were lost (e.g. flush completions that didn't
 * properly update recv_credits).  We reset recv_credits and
 * count_avail_recvmsg to their correct values so the pool is whole.
 */
static void smb_direct_reclaim_credits(struct smb_direct_transport *t)
{
	int posted, leaked;
	...
	spin_lock(&t->receive_credit_lock);
	/*
	 * After ib_drain_qp, all posted buffers have completed (either
	 * successfully or with flush errors).  Reset recv_credits to 0
	 * since nothing is actually posted to HW anymore.  Return all
	 * buffers to available pool.
	 */
	if (t->recv_credits > 0) {
		ksmbd_debug(RDMA,
			    "reclaiming %d outstanding recv credits (posted=%d)\n",
			    t->recv_credits, posted);
		t->count_avail_recvmsg += t->recv_credits;
		t->recv_credits = 0;
	}
	spin_unlock(&t->receive_credit_lock);

	/* Reset posted counter and check for leaks */
	posted = atomic_xchg(&t->recv_posted, 0);
	if (posted != 0)
		ksmbd_debug(RDMA,
			    "cleared %d orphaned recv_posted credits\n",
			    posted);

	leaked = atomic_read(&t->total_credits_granted) -
		 atomic_read(&t->total_credits_reclaimed);
	if (leaked != 0)
		pr_warn_ratelimited(
			"ksmbd: RDMA credit leak on teardown: "
			"granted=%d reclaimed=%d delta=%d\n",
			atomic_read(&t->total_credits_granted),
			atomic_read(&t->total_credits_reclaimed),
			leaked);
}
...
static void recv_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	/* Every completion (success or error) means one fewer posted buffer */
	atomic_dec(&t->recv_posted);
	atomic_inc(&t->total_credits_reclaimed);

	if (wc->status != IB_WC_SUCCESS || wc->opcode != IB_WC_RECV) {
		/*
		 * FIX: On flush errors (from ib_drain_qp during disconnect),
		 * we must decrement recv_credits to keep the invariant:
		 *   recv_credits + count_avail_recvmsg == recv_credit_max
		 *
		 * Previously, flush-error completions returned without
		 * adjusting recv_credits, causing a credit leak that could
		 * prevent new receive buffer postings on connection reuse.
		 */
		spin_lock(&t->receive_credit_lock);
		if (t->recv_credits > 0) {
			t->recv_credits--;
	...
	case SMB_DIRECT_MSG_DATA_TRANSFER: {
		...
		if (le16_to_cpu(data_transfer->credits_granted) > t->recv_credit_max)
			data_transfer->credits_granted = cpu_to_le16(t->recv_credit_max);
				...
				struct smb_direct_recvmsg *recvmsg)
{
	...
	/* Track outstanding posted buffers for leak detection */
	atomic_inc(&t->recv_posted);
	atomic_inc(&t->total_credits_granted);
...
again:
	...
	if (st->reassembly_data_length >= size) {
		...
		while (data_read < size) {
			...
			/*
			 * SMB Direct does not use RFC1002 framing on the wire.
			 * However, the upper SMB2 layer reads a 4-byte length
			 * prefix first (matching TCP transport behavior) and
			 * then issues a second read for the payload.  To avoid
			 * changing upper-layer logic, we synthesize a 4-byte
			 * big-endian length (data_length + remaining) on the
			 * first segment read when the caller asks for exactly
			 * 4 bytes.  This shim will be removed when a proper
			 * transport abstraction layer is introduced.
			 */
...
static void smb_direct_post_recv_credits(struct work_struct *work)
{
	...
	spin_lock(&t->receive_credit_lock);
	/*
	 * Bounds check: refuse to grant more credits than available.
	 * This should not happen in normal operation, but guards against
	 * accounting bugs.
	 */
	if (credits > t->count_avail_recvmsg) {
		pr_warn_ratelimited(
			"ksmbd: RDMA credit overcommit: "
			"trying to grant %d but only %d available\n",
			credits, t->count_avail_recvmsg);
		credits = t->count_avail_recvmsg;
	}
	...
	if (t->count_avail_recvmsg <
	    t->recv_credit_max * SMB_DIRECT_CREDIT_LOW_WATERMARK_PCT / 100)
		pr_warn_ratelimited(
			"ksmbd: RDMA credit pool low: "
			"avail=%d recv_credits=%d max=%d\n",
			t->count_avail_recvmsg,
			t->recv_credits, t->recv_credit_max);
	spin_unlock(&t->receive_credit_lock);
			     ...
			     bool need_invalidate, unsigned int remote_key)
{
	...
	/*
	 * SMB Direct (RDMA) does not use RFC1002 transport framing.
	 * However, the upper layer (SMB2 response code) prepends a
	 * 4-byte RFC1002 length header in iov[0] for TCP compatibility.
	 * We must skip this first iov segment since the SMB Direct
	 * data transfer header provides its own framing via the
	 * DataLength and RemainingDataLength fields.
	 *
	 * The WARN_ON_ONCE guards against callers that don't follow
	 * the expected iov layout (iov[0] = 4-byte RFC1002 header,
	 * iov[1..n] = actual SMB2 payload).
	 */
...
/**
 * smb_direct_rdma_transform_needed - Check if RDMA transform is required
 * @conn: connection to check
 *
 * Returns true if the connection has negotiated encryption and RDMA
 * payloads need to be wrapped with a transform header per MS-SMB2 2.2.43.
 */
static bool smb_direct_rdma_transform_needed(struct ksmbd_conn *conn)
{
	if (!conn)
		return false;
...
/**
 * smb_direct_rdma_encrypt - Encrypt RDMA payload with transform header
 * @t:          RDMA transport
 * @buf:        plaintext data buffer
 * @buf_len:    length of plaintext data
 * @out_buf:    output buffer with transform header + encrypted data
 * @out_len:    output total length
 *
 * Prepends an SMB2 RDMA Transform Header (MS-SMB2 2.2.43) followed by
 * an encryption transform entry, then encrypts the payload using the
 * session's encryption key (AES-CCM or AES-GCM matching the connection's
 * cipher_type). The encrypted data is placed after the headers.
 *
 * Returns 0 on success, negative errno on failure.
 * Caller must kfree(*out_buf) on success.
 */
static int smb_direct_rdma_encrypt(struct smb_direct_transport *t,
...
/**
 * smb_direct_rdma_decrypt - Decrypt RDMA payload with transform header
 * @t:          RDMA transport
 * @buf:        buffer containing transform header + encrypted data
 * @buf_len:    total length of buffer
 * @out_buf:    output plaintext buffer (within buf, modified in-place)
 * @out_len:    output plaintext length
 *
 * Verifies and strips the SMB2 RDMA Transform Header, then decrypts
 * the payload using the session's decryption key.
 *
 * Returns 0 on success, negative errno on failure.
 * On success, *out_buf points into the original buf (in-place decrypt)
 * and *out_len is the plaintext length.
 */
static int smb_direct_rdma_decrypt(struct smb_direct_transport *t,
				   void *buf, int buf_len,
				   void **out_buf, int *out_len)
{
	struct ksmbd_conn *conn = KSMBD_TRANS(t)->conn;
				 ...
				 unsigned int desc_len)
{
	...
	/*
	 * RDMA write = server pushes data to client's memory.
	 * If encryption is negotiated, encrypt the payload and prepend
				...
				unsigned int desc_len)
{
	...
	/*
	 * RDMA read = server pulls data from client's memory.
	 * If encryption is negotiated, the client will have wrapped
	 * the data with an RDMA Transform Header. After reading,
	 * verify and decrypt the payload per MS-SMB2 2.2.43.
	 */
	if (smb_direct_rdma_transform_needed(t->conn)) {
		void *plain_buf;
		int plain_len;
		...
		ret = smb_direct_rdma_decrypt(st, buf, buflen,
					      &plain_buf, &plain_len);
		if (ret) {
			pr_err("RDMA read: transform decryption failed: %d\n",
			       ret);
			return ret;
		}

		/*
		 * If the plaintext was decrypted in place at an offset,
		 * move it to the start of the buffer so the caller
		 * finds the data at buf[0].
		 */
		if (plain_buf != buf && plain_len > 0)
			memmove(buf, plain_buf, plain_len);

		return 0;
	}

	return smb_direct_rdma_xmit(st, buf, buflen,
				    desc, desc_len, true);
}
				 ...
				 struct rdma_cm_event *event)
{
	...
	default:
		pr_err("Unexpected RDMA CM event. cm_id=%pK, event=%s (%d)\n",
				  ...
				  struct ib_qp_cap *cap)
{
	...
	if (t->pages_per_rw_credit <= 1)
		t->pages_per_rw_credit = 2;
...
static int smb_direct_prepare(struct ksmbd_transport *t)
{
	...
	/*
	 * Enforce minimum buffer sizes to prevent a malicious client from
	 * forcing pathologically small buffers that cause excessive
	 * fragmentation and degrade server performance (DoS).
	 * 8192 accommodates a full SMB2 header plus reasonable payload.
	 */
	if (st->max_recv_size < 8192) {
		pr_err("RDMA: client max_recv_size too small: %d\n",
		       st->max_recv_size);
		ret = -ECONNABORTED;
		goto out;
	}

	if (st->max_send_size < 8192) {
		pr_err("RDMA: client max_send_size too small: %d\n",
		       st->max_send_size);
		ret = -ECONNABORTED;
		goto out;
	}
	...
	st->max_fragmented_send_size =
		le32_to_cpu(req->max_fragmented_size);
	if (st->max_fragmented_send_size == 0)
		st->max_fragmented_send_size = smb_direct_max_fragmented_recv_size;
	else if (st->max_fragmented_send_size < 131072)
		st->max_fragmented_send_size = 131072;
...
static int smb_direct_handle_connect_request(struct rdma_cm_id *new_cm_id)
{
	...
	if (server_conf.max_connections &&
	    atomic_inc_return(&smbd_active_conn) > server_conf.max_connections) {
		atomic_dec(&smbd_active_conn);
		pr_err_ratelimited("RDMA: max connections reached\n");
				     ...
				     struct rdma_cm_event *event)
{
	...
	default:
		pr_err("Unexpected listen event. cm_id=%pK, event=%s (%d)\n",
		       cm_id, rdma_event_msg(event->event), event->event);
		break;
	}
	return 0;
