...
/*
 * SMB1_SERVER_CAPS: advertised server capabilities.
 * NOTE: CAP_LOCK_AND_READ (0x00000100) is intentionally omitted — opcode
 * 0x13 (SMB_COM_LOCK_AND_READ) has no handler, so advertising the capability
 * would be a protocol violation.
 * P1.27: Added CAP_MPX_MODE (multiplexing supported, MaxMpxCount=10),
 *        CAP_RPC_REMOTE_APIS (IPC$/named-pipe DCE/RPC implemented),
 *        CAP_INFOLEVEL_PASSTHRU (TRANS2 passthrough info levels handled).
 */
...
/*
 * P1.8: STATUS_SMB_BAD_TID — NTSTATUS for invalid TID in SMB1 tree
 * disconnect and related operations.  Windows maps ERRSrv/ERRinvnid
 * to NT status 0xC000004B (STATUS_INVALID_NETWORK_RESPONSE is close;
 * Samba uses STATUS_SMB_BAD_TID = 0xC0000006 is incorrect).
 * Use STATUS_NETWORK_NAME_DELETED (0xC00000C9) per common practice.
 */
#define STATUS_SMB_BAD_TID cpu_to_le32(0xC00000C9)
...
#define NT_TRANSACT_MAX_SUBCOMMAND    0x08

/*
 * NT_TRANSACT request (MS-CIFS 2.2.4.62)
 * WordCount is variable: 19 + SetupCount
 * Note: ParameterCount/DataCount are 32-bit (unlike TRANS2's 16-bit).
 */
struct smb_com_nt_transact_req {
	...
	__le32 DataOffset;
	__u8 SetupCount;
	__le16 Function;	/* NT_TRANSACT subcommand */
	__u8 Buffer[];
} __packed;
 ...
 *
 * Return: 0 on valid, -EINVAL if any region overflows.
 */
static inline int smb1_validate_nt_transact_buffer(const void *request_buf,
						   unsigned int buf_len)
{
	const struct smb_com_nt_transact_req *req = request_buf;
	unsigned int param_off, param_cnt, data_off, data_cnt;
	unsigned int total_param, total_data;

	param_off = le32_to_cpu(req->ParameterOffset);
	param_cnt = le32_to_cpu(req->ParameterCount);
	data_off  = le32_to_cpu(req->DataOffset);
	data_cnt  = le32_to_cpu(req->DataCount);
	total_param = le32_to_cpu(req->TotalParameterCount);
	total_data  = le32_to_cpu(req->TotalDataCount);

	/* Integer overflow guards: check individual values first */
	if (param_off > buf_len || param_cnt > buf_len ||
	    data_off > buf_len || data_cnt > buf_len)
		return -EINVAL;

	if (param_cnt > 0) {
		if (param_off + 4 > buf_len ||
		    param_off + 4 + param_cnt > buf_len)
			return -EINVAL;
	}

	if (data_cnt > 0) {
		if (data_off + 4 > buf_len ||
		    data_off + 4 + data_cnt > buf_len)
			return -EINVAL;
	}

	/* Total must be >= current count */
	if (total_param < param_cnt || total_data < data_cnt)
		return -EINVAL;

	/* SetupCount must be within buffer */
	if (req->SetupCount > 0) {
		unsigned int setup_end = offsetof(struct smb_com_nt_transact_req,
						  Buffer) +
					 req->SetupCount * sizeof(__le16);
		if (setup_end > buf_len)
			return -EINVAL;
	}
...
#define SMB_COM_FIND                  0x0C /* obsolete search */
#define SMB_COM_FIND_UNIQUE           0x16 /* obsolete unique search */
/* A.2: P2 missing commands */
...
/* NT_TRANSACT (0xA0) request/response structures (MS-SMB §2.2.4.62) */
struct smb_com_ntransact_req {
	struct smb_hdr hdr;		/* wct = 19+SetupCount */
	__u8  MaxSetupCount;
	__u16 Reserved;
	__le32 TotalParameterCount;
	__le32 TotalDataCount;
	...
	__u8  SetupCount;
	__le16 Function;
	__le16 SetupWords[1];	/* variable: SetupCount words */
	/* followed by ByteCount(__le16) + Pad + Parameters + Data */
} __packed;
...
extern int smb_nt_transact(struct ksmbd_work *work);
...
extern int smb_nt_transact(struct ksmbd_work *work);
extern int smb_find(struct ksmbd_work *work);
extern int smb_process_exit(struct ksmbd_work *work);
extern int smb_find(struct ksmbd_work *work);
/* A.2: P2 missing commands */
extern int smb_query_information2(struct ksmbd_work *work);
extern int smb_set_information2(struct ksmbd_work *work);

/* A.2: Request/Response structures for QUERY_INFORMATION2 (0x23) */
...
/* A.2: Request/Response structures for SET_INFORMATION2 (0x22) */
