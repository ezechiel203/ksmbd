...
/*
 * ksmbd-internal error code for share access violations.
 * Value 50000 is intentionally far above the kernel errno range (< 4096)
 * to avoid conflicts. This must never be exposed to userspace or returned
 * as a syscall error — it is translated to NT_STATUS_SHARING_VIOLATION
...
struct file_id_extd_both_dir_info {
	__le32 NextEntryOffset;
	__u32 FileIndex;
	__le64 CreationTime;
	__le64 LastAccessTime;
...
struct file_id_64_extd_dir_info {
	__le32 NextEntryOffset;
	__u32 FileIndex;
	__le64 CreationTime;
	__le64 LastAccessTime;
...
struct file_id_64_extd_both_dir_info {
	__le32 NextEntryOffset;
	__u32 FileIndex;
	__le64 CreationTime;
	__le64 LastAccessTime;
...
struct file_id_all_extd_dir_info {
	__le32 NextEntryOffset;
	__u32 FileIndex;
	__le64 CreationTime;
	__le64 LastAccessTime;
...
struct file_id_all_extd_both_dir_info {
	__le32 NextEntryOffset;
	__u32 FileIndex;
	__le64 CreationTime;
	__le64 LastAccessTime;
...
/* Size of the RFC1002 4-byte length prefix preceding each SMB PDU */
#define RFC1002_HEADER_LEN	4
...
{
	unsigned int cur = get_rfc1002_len(buf);

	if (WARN_ON_ONCE((unsigned int)(cur + count) > MAX_STREAM_PROT_LEN))
		return;
