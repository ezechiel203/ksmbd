...
/* SMB 3.1.1 server-to-client notification (MS-SMB2 §2.2.44) */
#define SMB2_SERVER_TO_CLIENT_NOTIFICATION_HE	0x0013
...
#define SMB2_SERVER_TO_CLIENT_NOTIFICATION \
	cpu_to_le16(SMB2_SERVER_TO_CLIENT_NOTIFICATION_HE)
...
/*
 * Maximum SMB2 header + fixed request size:
 *   4 bytes RFC1002 length prefix
 * + 64 bytes SMB2 header (MS-SMB2 §2.2.1)
 * + 48 bytes worst-case fixed request body (2 * 24 words)
 * + 2 bytes ByteCount
 * + 2 bytes padding
 * = 120 bytes (0x78)
 */
...
/* SMB2 Transform header Flags field (MS-SMB2 §2.2.41) */
...
#define SMB2_GLOBAL_CAP_NOTIFICATIONS	0x00000080 /* New to SMB3.1.1 (MS-SMB2 §2.2.4) */
/* Internal types */
...
/*
 * WARNING: SMB3_COMPRESS_LZ4 (0x0005) is NOT defined in the MS-SMB2
 * specification.  Value 0x0005 is unassigned in all known spec revisions.
 * This constant MUST NOT be advertised to clients in a
 * SMB2_COMPRESSION_CAPABILITIES negotiate context response, as doing so
 * would break interoperability with compliant Windows/Samba clients (which
 * will never offer 0x0005).  If a future spec revision assigns 0x0005 to a
 * different algorithm, advertising LZ4 as 0x0005 would silently misidentify
 * that algorithm.  Negotiate code (smb2_negotiate.c) MUST NOT include this
 * value in the list of algorithms offered to clients.
 */
#define SMB3_COMPRESS_LZ4	cpu_to_le16(0x0005) /* NON-SPEC: not in MS-SMB2 */
...
/* SMB 3.1.1 TREE_CONNECT Flags (overloads Reserved field, MS-SMB2 §2.2.9) */
...
/* SMB2 READ Flags (MS-SMB2 §2.2.19) */
#define SMB2_READFLAG_READ_UNBUFFERED              0x00000001
#define SMB2_READFLAG_READ_COMPRESSED              0x00000002
#define SMB2_READFLAG_REQUEST_TRANSPORT_ENCRYPTION 0x00000004

/* For write request Flags field (MS-SMB2 §2.2.21) */
#define SMB2_WRITEFLAG_WRITE_THROUGH               0x00000001
#define SMB2_WRITEFLAG_WRITE_UNBUFFERED            0x00000002
#define SMB2_WRITEFLAG_REQUEST_TRANSPORT_ENCRYPTION 0x00000004
...
/* MS-SMB2 2.2.31.2 - SRV_READ_HASH Request */
...
struct srv_read_hash_req {
	__le32	HashType;
	__le32	HashVersion;
	__le32	HashRetrievalType;
	__le64	Length;
	__le64	Offset;
} __packed;

/* MS-SMB2 2.2.32.4 - SRV_READ_HASH Response (V1) */
struct srv_read_hash_rsp_v1 {
	__le64	Offset;
	__le32	BufferLength;
	__u8	Buffer[];	/* Content Information */
} __packed;

/* MS-SMB2 2.2.32.4.1 - SRV_READ_HASH Response (V2) */
struct srv_read_hash_rsp_v2 {
	__le64	Offset;
	__le32	BufferLength;
	__u8	Buffer[];	/* Content Information */
} __packed;

/* FSCTL_PIPE_WAIT Request - MS-SMB2 / MS-FSCC */
struct fsctl_pipe_wait_req {
	__le64	Timeout;	/* In 100-ns units */
	__le32	NameLength;	/* In bytes */
	__u8	TimeoutSpecified;
	__u8	Padding;
	__u8	Name[];		/* Unicode pipe name */
} __packed;
...
#define SMB2_LOCKFLAG_MASK		(SMB2_LOCKFLAG_SHARED | \
...
struct smb2_lock_req {
	 ...
	 * Bits 0-3:   LockSequenceNumber (4 bits)
	 * Bits 4-7:   LockSequenceIndex (4 bits)
	 * Bits 8-31:  Reserved
	 */
...
#define FILE_PROCESS_IDS_USING_FILE_INFORMATION_SIZE 8
#define FILE_NETWORK_PHYSICAL_NAME_INFORMATION_SIZE  4
#define FILE_IS_REMOTE_DEVICE_INFORMATION_SIZE       4
...
#define FILE_REMOTE_PROTOCOL_INFORMATION_SIZE 116
#define FILE_VOLUME_NAME_INFORMATION_SIZE     4
#define FILE_DISPOSITION_INFORMATION_EX_SIZE  4
#define FILE_CASE_SENSITIVE_INFORMATION_SIZE  4

/* FS Info response  size */
#define FS_LABEL_INFORMATION_SIZE      4
...
#define FS_DRIVER_PATH_INFORMATION_SIZE 4
...
/*
 * fs_type_info is not a wire-format struct, so __packed is not needed.
 * Removing __packed avoids alignment issues from the pointer field.
 */
struct fs_type_info {
	char		*fs_name;
	long		magic_number;
...
/* MS-SMB2 §2.2.23.2 BreakReason values */
#define SMB2_LEASE_BREAK_REASON_NONE				cpu_to_le32(0x00)
#define SMB2_LEASE_BREAK_REASON_PARENT_LEASE_KEY_CHANGED	cpu_to_le32(0x02)
...
/*
 * SMB2 SERVER_TO_CLIENT_NOTIFICATION (MS-SMB2 §2.2.44)
 * Server-initiated, unsolicited notification to client (SMB 3.1.1+).
 */
#define SMB2_NOTIFY_SESSION_CLOSED	cpu_to_le32(0x00000000)

struct smb2_server_to_client_notification {
	struct smb2_hdr hdr;
	__le16 StructureSize;     /* Must be 4 */
	__le16 Reserved;          /* MBZ */
	__le32 NotificationType;  /* e.g. SMB2_NOTIFY_SESSION_CLOSED */
	/* NotificationData follows (type-specific, variable length) */
} __packed;
...
#define FILE_PROCESS_IDS_USING_FILE_INFORMATION 47
#define FILE_NORMALIZED_NAME_INFORMATION 48
#define FILE_NETWORK_PHYSICAL_NAME_INFORMATION 49
#define FILEID_GLOBAL_TX_DIRECTORY_INFORMATION 50
#define FILE_IS_REMOTE_DEVICE_INFORMATION 51
#define FILE_STANDARD_LINK_INFORMATION	54
#define FILE_REMOTE_PROTOCOL_INFORMATION 55
#define FILE_RENAME_INFORMATION_BYPASS_ACCESS_CHECK 56
#define FILE_LINK_INFORMATION_BYPASS_ACCESS_CHECK 57
#define FILE_VOLUME_NAME_INFORMATION	58
#define FILE_ID_INFORMATION		59
#define FILEID_EXTD_DIRECTORY_INFORMATION 60
#define FILEID_EXTD_BOTH_DIRECTORY_INFORMATION 63
#define FILE_DISPOSITION_INFORMATION_EX	64	/* POSIX unlink semantics */
#define FILE_RENAME_INFORMATION_EX	65	/* POSIX atomic rename semantics */
#define FILE_RENAME_INFORMATION_EX_BYPASS_ACCESS_CHECK 66
#define FILE_STAT_INFORMATION		68  /* MS-FSCC §2.4.43 */
#define FILE_STAT_LX_INFORMATION	70  /* MS-FSCC §2.4.44 (Linux ext) */
#define FILE_CASE_SENSITIVE_INFORMATION	71  /* MS-FSCC §2.4.8 */
#define FILEID_64_EXTD_DIRECTORY_INFORMATION 78
#define FILEID_64_EXTD_BOTH_DIRECTORY_INFORMATION 79
#define FILEID_ALL_EXTD_DIRECTORY_INFORMATION 80
#define FILEID_ALL_EXTD_BOTH_DIRECTORY_INFORMATION 81
...
/* FileValidDataLengthInformation (class 39) */
struct smb2_file_valid_data_length_info {
	__le64 ValidDataLength;
} __packed;

/* FileObjectIdInformation (class 29) — full 64-byte layout per MS-FSCC §2.4.26 */
/* XATTR_NAME_OBJECT_ID is defined in xattr.h */
struct smb2_file_objectid_info {
	__u8 ObjectId[16];
	__u8 ExtendedInfo[48]; /* BirthVolumeId(16)+BirthObjectId(16)+DomainId(16) */
} __packed;
...
/* FileDispositionInformationEx (class 64, MS-FSCC §2.4.11.1) */
#define FILE_DISPOSITION_EX_DELETE		0x00000001
#define FILE_DISPOSITION_EX_POSIX_SEMANTICS	0x00000002
#define FILE_DISPOSITION_EX_FORCE_IMAGE_SECTION	0x00000004
#define FILE_DISPOSITION_EX_ON_CLOSE		0x00000008
#define FILE_DISPOSITION_EX_IGNORE_READONLY	0x00000010
...
/*
 * smb2_file_stat_info and smb2_file_stat_lx_info are defined locally in
 * ksmbd_info.c to match the exact wire format used by the Linux SMB client.
 * Do not add duplicate definitions here to avoid redefinition conflicts.
 */
...
struct smb2_posix_info {
	...
	__le32 Mode;
	/*
	 * SidBuffer contain two sids(owner sid(up to 28), group sid(16)).
	 * Use 44 bytes to match create_posix_rsp and accommodate domain SIDs.
	 */
	u8 SidBuffer[44];
	__le32 name_len;
	u8 name[];
...
void smb2_send_session_closed_notification(struct ksmbd_work *work);
...
/* Negotiate context decode/assemble (smb2_negotiate.c) */
...
/* Session setup helpers (smb2_session.c) */
...
/* Lock helpers (smb2_lock.c) */
struct ksmbd_lock;
int smb2_set_flock_flags(struct file_lock *flock, int flags);
struct ksmbd_lock *smb2_lock_init(struct file_lock *flock,
				  unsigned int cmd, int flags,
				  unsigned long long smb_start,
				  unsigned long long smb_end,
				  struct list_head *lock_list);
int check_lock_sequence(struct ksmbd_file *fp, __le32 lock_seq_val);
void store_lock_sequence(struct ksmbd_file *fp, __le32 lock_seq_val);

/* Create helpers (smb2_create.c) */
int smb2_create_open_flags(bool file_present, __le32 access,
			   __le32 disposition, int *may_flags,
			   __le32 coptions, umode_t mode);
int smb_check_parent_dacl_deny(struct ksmbd_conn *conn,
			       const struct path *parent,
			       int uid, bool is_dir);
int smb2_create_sd_buffer(struct ksmbd_work *work,
...
/* PDU common helpers (smb2_pdu_common.c) */
void init_chained_smb2_rsp(struct ksmbd_work *work);
