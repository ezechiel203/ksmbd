...
/* Defined in uniupr.h, included only from unicode.c */
...
/*
 * UniStrcat:  Concatenate the second string to the first (bounded)
 *
 * Returns:
 *     Address of the first string
 */
static inline wchar_t *UniStrcat(wchar_t *ucs1, size_t dest_size,
				 const wchar_t *ucs2)
{
	wchar_t *anchor = ucs1;
	size_t len1 = UniStrnlen(ucs1, dest_size);
	size_t remaining;

	if (len1 >= dest_size)
		return ucs1;

	remaining = dest_size - len1;
	ucs1 += len1;
	while (remaining > 1 && *ucs2) {
		*ucs1++ = *ucs2++;
		remaining--;
	}
	*ucs1 = 0;
...
/*
 * UniStrcpy:  Copy a string (bounded)
 */
static inline wchar_t *UniStrcpy(wchar_t *ucs1, size_t dest_size,
				 const wchar_t *ucs2)
{
	wchar_t *anchor = ucs1;
...
static inline wchar_t UniToupper(register wchar_t uc)
{
	...
	if (uc >= 0 && (size_t)uc < sizeof(SmbUniUpperTable)) {
		/* Latin characters */
