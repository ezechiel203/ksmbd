...
/*
 * Domain-aware SID-to-UID mapping constants.
 *
 * SECURITY: Without domain validation, two different AD domains that
 * assign the same RID to different users would silently map to the
 * same Linux UID, causing ACL mis-application and quota bypass.
 *
 * When a SID's domain prefix does not match the server's configured
 * domain SID, we apply a hash-based offset to the RID so that
 * different domains produce different UIDs for the same RID.
 *
 * The multiplier is chosen to be large enough to separate domains
 * while small enough to stay within uid_t range for typical RID values.
 */
#define DOMAIN_UID_OFFSET_MULTIPLIER	100000U
#define DOMAIN_UID_MAX_OFFSET		(U32_MAX - DOMAIN_UID_OFFSET_MULTIPLIER)

/* Return value when a SID cannot be mapped (NT_STATUS_NONE_MAPPED) */
#define KSMBD_STATUS_NONE_MAPPED	(-ENOENT)
...
#endif
		   struct smb_ntsd *ppntsd, int ppntsd_size,
		   int addition_info, __u32 *secdesclen,
		   struct smb_fattr *fattr, unsigned int buf_size);
...
/* ACL-02: parse_sid must be visible outside CONFIG_KUNIT for use in parse_dacl */
