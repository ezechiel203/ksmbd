...
/*
 * VSS-01: Wire stride for UTF-16LE snapshot strings.
 * Each entry on the wire is exactly 24 ASCII chars + NUL = 25 UTF-16LE
 * elements (50 bytes).  KSMBD_VSS_GMT_TOKEN_LEN is larger for storage
 * headroom only and must NOT be used as the wire stride.
 */
#define KSMBD_VSS_GMT_WIRE_CHARS	24	/* @GMT-YYYY.MM.DD-HH.MM.SS */
#define KSMBD_VSS_GMT_WIRE_LEN		(KSMBD_VSS_GMT_WIRE_CHARS + 1)	/* +NUL */
