...
#define KSMBD_REQ_MAX_ACCOUNT_NAME_SZ	256
#define KSMBD_REQ_MAX_HASH_SZ		18
#define KSMBD_REQ_MAX_SHARE_NAME	64

#define KSMBD_MAX_SHARE_PAYLOAD_SZ	(64 * 1024)
#define KSMBD_MAX_SPNEGO_BLOB_SZ	(64 * 1024)
#define KSMBD_MAX_RPC_PAYLOAD_SZ	(256 * 1024)
 ...
 *
 * ifc_list_sz: size of interface list in ____payload[]; must not exceed 4KB.
 * Callers must validate ifc_list_sz before accessing payload data.
 */

/* NETLINK-01: Maximum valid size for ifc_list_sz in ksmbd_startup_request */
#define KSMBD_REQ_MAX_IFC_LIST_SZ	4096
 ...
 *
 * payload_sz: total size of ____payload[]; must not exceed
 * KSMBD_MAX_SHARE_PAYLOAD_SZ (64KB).
 * veto_list_sz: size of veto list within ____payload[]; must be < payload_sz.
 * Callers must validate both sizes before accessing payload data.
 */
 ...
 * IPC user daemon.
 *
 * Maximum expected payload_sz: 256KB (KSMBD_MAX_RPC_PAYLOAD_SZ).
 * Callers must validate payload_sz before allocating or copying data.
 */
struct ksmbd_rpc_command {
	__u32	handle;
	__u32	xid;
	__u32	flags;
	__u32	payload_sz;	/* must not exceed KSMBD_MAX_RPC_PAYLOAD_SZ */
	__u8	payload[];
};

/*
 * RPC named-pipe state returned for FSCTL_PIPE_PEEK.
 *
 * pipe_state mirrors the FILE_PIPE_*_STATE values in MS-FSCC §2.3.32.
 */
...
struct ksmbd_spnego_authen_request {
	...
	__u16	spnego_blob_len;	/*
					 * the length of spnego_blob.
					 * NOTE: __u16 limits SPNEGO blobs to 64KB,
					 * which may be insufficient for complex
					 * Kerberos configurations with many PAC
					 * entries or cross-realm trust chains.
					 */
	__u8	spnego_blob[];		/*
...
enum ksmbd_event {
	...
	KSMBD_EVENT_SHARE_CONFIG_FLUSH		= 25,
...
/* M-10: Offline caching mode bits (21-22); maps to SMB2 ShareFlags caching bits [5:4] */
#define KSMBD_SHARE_FLAG_CACHING_MANUAL			(0 << 21)
#define KSMBD_SHARE_FLAG_CACHING_AUTO			BIT(21)
