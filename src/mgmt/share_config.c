	...
	__must_hold(RCU)
{
	...
	lockdep_assert(rcu_read_lock_held() ||
		       lockdep_is_held(&shares_table_lock));
						       ...
						       const char *name)
{
	...
	/* M-10: translate netlink caching bits (21-22) to SMB2 ShareFlags caching bits [5:4] */
	switch (resp->flags & KSMBD_SHARE_FLAG_CACHING_MASK) {
	case KSMBD_SHARE_FLAG_CACHING_AUTO:
		share->offline_caching_mode = 0x00000010; /* SMB2_SHAREFLAG_AUTO_CACHING */
		break;
	case KSMBD_SHARE_FLAG_CACHING_VDO:
		share->offline_caching_mode = 0x00000020; /* SMB2_SHAREFLAG_VDO_CACHING */
		break;
	case KSMBD_SHARE_FLAG_CACHING_NONE:
		share->offline_caching_mode = 0x00000030; /* SMB2_SHAREFLAG_NO_CACHING */
		break;
	default: /* KSMBD_SHARE_FLAG_CACHING_MANUAL or unset — default is 0 from kzalloc */
		break;
	}
	...
	/*
	 * Enable streams by default for non-pipe shares.
	 * ksmbdctl may not set this flag, but ADS support
	 * is expected for normal file shares.
	 */
	if (!test_share_config_flag(share, KSMBD_SHARE_FLAG_PIPE))
		share->flags |= KSMBD_SHARE_FLAG_STREAMS;
	...
	if (!test_share_config_flag(share, KSMBD_SHARE_FLAG_PIPE)) {
		...
		{
			char *spath = ksmbd_share_config_path(resp);

			if (!spath)
				goto out_bad_share;
			share->path = kstrndup(spath, path_len,
					      KSMBD_DEFAULT_GFP);
		}
...
void ksmbd_share_configs_flush(void)
{
	...
	/*
	 * MGMT-01: Use hash_del_rcu() + call_rcu() to avoid UAF when
	 * __share_lookup_rcu() RCU readers hold a pointer to a share being
	 * freed.  Mirrors the pattern used in __ksmbd_share_config_put().
	 */
	spin_lock(&shares_table_lock);
	hash_for_each_safe(shares_table, bkt, tmp, share, hlist) {
		/*
		 * Only remove idle entries (refcount == 1 means no active
		 * tree connects hold a reference).
		 */
		if (refcount_read(&share->refcount) == 1) {
			hash_del_rcu(&share->hlist);
			call_rcu(&share->rcu_head, kill_share_rcu);
		}
	}
