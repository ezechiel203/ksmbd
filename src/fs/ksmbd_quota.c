 ...
 * @sid:     the Windows SID
 * @sid_len: byte length of the SID buffer
 * @uid_out: [out] the resulting uid
 *
 * Uses the domain-aware SID-to-UID mapping from smbacl.c to convert
 * a Windows SID to a Linux UID.  This ensures that SIDs from
 * different domains with the same RID map to different UIDs,
 * preventing quota bypass in multi-domain AD environments.
 *
 * Return: 0 on success, negative errno on failure
 */
static int ksmbd_sid_to_uid(const struct smb_sid *sid, unsigned int sid_len,
			    uid_t *uid_out)
{
	...
	/* Validate that the buffer is large enough for all sub-authorities */
	if (sid_len < CIFS_SID_BASE_SIZE +
		      (unsigned int)sid->num_subauth * sizeof(__le32))
		return -EINVAL;

	/*
	 * Use domain-aware mapping: the RID is offset by a hash of the
	 * domain prefix for foreign-domain SIDs, preventing UID collisions.
	 */
			     ...
			     unsigned int *out_len)
{
	...
	*out_len = 0;
	...
	{
		unsigned int ibo = le16_to_cpu(req->InputBufferOffset);

		if (ibo < sizeof(struct smb2_hdr) ||
		    ibo - sizeof(struct smb2_hdr) + sizeof(struct smb2_query_quota_info) >
		    input_buf_len + sizeof(struct smb2_hdr)) {
			ksmbd_debug(SMB,
				    "Quota query: InputBufferOffset %u out of bounds\n",
				    ibo);
			*out_len = 0;
			return 0;
		}
		input_buf = (void *)req + ibo - sizeof(struct smb2_hdr);
	}
	...
	if (le32_to_cpu(qqi->SidListLength) > 0) {
		...
		if (sizeof(*qqi) + sid_list_len > input_buf_len) {
			ksmbd_debug(SMB,
				    "Quota query: SidListLength %u exceeds buffer (%u)\n",
				    sid_list_len, input_buf_len);
			*out_len = 0;
			return 0;
		}
		...
		while (offset < sid_list_len) {
			...
			ret = ksmbd_sid_to_uid(sid, sid_len, &uid);
			if (ret) {
				ksmbd_debug(SMB,
		...
		unsigned int start_sid_end;
		struct smb_sid *sid;
		unsigned int entry_len;
		uid_t uid;
		int ret;

		if (check_add_overflow(start_sid_off, start_sid_len,
				       &start_sid_end) ||
		    start_sid_end > input_buf_len ||
		    start_sid_len < CIFS_SID_BASE_SIZE)
			goto done;

		sid = (struct smb_sid *)((u8 *)qqi + start_sid_off);

		ret = ksmbd_sid_to_uid(sid, start_sid_len, &uid);
		if (ret)
			goto done;
