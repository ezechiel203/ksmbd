					 ...
					 unsigned int *out_len)
{
	...
	mutex_lock(&vss_backend_lock);
	list_for_each_entry(be, &vss_backend_list, list) {
		ret = be->enumerate(share->path, &snap_list);
	 ...
	 * Each @GMT token is 24 chars + NUL in UTF-16LE = 50 bytes
	 * Plus a final NUL terminator (2 bytes)
	 */
	array_size =
		snap_list.count * (KSMBD_VSS_GMT_TOKEN_LEN * 2) +
		sizeof(__le16);
	...
	for (i = 0; i < snap_list.count; i++) {
		...
		/* Convert ASCII @GMT token to UTF-16LE */
		for (j = 0; j < KSMBD_VSS_GMT_TOKEN_LEN - 1; j++)
			utf16_ptr[j] = cpu_to_le16((u16)gmt[j]);
		/* NUL terminator */
		utf16_ptr[KSMBD_VSS_GMT_TOKEN_LEN - 1] =
			cpu_to_le16(0);

		utf16_ptr += KSMBD_VSS_GMT_TOKEN_LEN;
		returned++;
	}
