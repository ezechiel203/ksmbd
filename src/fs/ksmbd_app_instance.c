 ...
 */
static void close_previous_app_instance(struct ksmbd_file *fp,
					u64 version_high,
					u64 version_low,
					bool has_version)
{
	...
	/*
	 * Serialize access to the fp list with ci->m_lock.  We capture
	 * the volatile_id and the file_table pointer under the lock, then
	 * close using ksmbd_force_close_fd() after releasing it.
	 *
	 * ksmbd_close_fd() would use work->sess->file_table, which is
	 * wrong when the previous open belongs to a different session
	 * (e.g. a different client connection).  ksmbd_force_close_fd()
	 * uses the explicit ft pointer captured from prev_fp->ft, which
	 * is the file_table of the session that originally opened the file.
	 *
	 * prev_fp->ft is stable for this lifetime because a live file handle
	 * in that file_table keeps its owning session alive (sessions are
	 * not freed while they still have open file handles).
	 */
	down_read(&ci->m_lock);
	list_for_each_entry(prev_fp, &ci->m_fp_list, node) {
		...
		/*
		 * Capture the volatile ID and file_table pointer under
		 * ci->m_lock.  The ft pointer is stable as long as the
		 * file handle lives (which it does, since we hold m_lock).
		 */
	...
	if (has_file_id(found_id) && found_ft) {
		ksmbd_debug(SMB,
			    "Closing previous app instance open fid=%llu\n",
			    found_id);
		ksmbd_force_close_fd(found_ft, found_id);
	}
}
				      ...
				      unsigned int ctx_len)
{
	...
	close_previous_app_instance(fp,
					   ...
					   unsigned int ctx_len)
{
	 ...
	 */
	ver_high = get_unaligned_le64(data +
				      APP_INSTANCE_VERSION_HIGH_OFFSET);
	ver_low = get_unaligned_le64(data +
	...
	if (fp->has_app_instance_id)
		close_previous_app_instance(fp, ver_high, ver_low, true);
