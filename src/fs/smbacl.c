...
static __u16 ksmbd_min_ace_size(void)
{
	return offsetof(struct smb_ace, sid) + 8;
}

static int ksmbd_validate_ace(const struct smb_ace *ace, int remaining,
			      __u16 *ace_size)
{
	__u16 size;

	if (remaining < ksmbd_min_ace_size())
		return -EINVAL;

	size = le16_to_cpu(ace->size);
	if (size < ksmbd_min_ace_size() || size > remaining)
		return -EINVAL;

	if (parse_sid((struct smb_sid *)&ace->sid, (char *)ace + size))
		return -EINVAL;
		     ...
		     unsigned int uid, unsigned int gid)
{
	 ...
	 */
	{
		...
		scan_ace = (struct smb_ace *)((char *)parent_pdacl +
				sizeof(struct smb_acl));
		scan_size = pdacl_size - sizeof(struct smb_acl);
		for (i = 0; i < num_aces; i++) {
			__u16 pace_sz;

			if (ksmbd_validate_ace(scan_ace, scan_size, &pace_sz))
				break;
			{
				size_t inherited_sz;
	...
	for (i = 0; i < num_aces; i++) {
		...
		if (ksmbd_validate_ace(parent_aces, aces_size, &pace_size))
			break;
...
next_ace:
	...
	if (*pdaccess & FILE_MAXIMAL_ACCESS_LE && (found || others_ace)) {
		granted = READ_CONTROL | WRITE_DAC | FILE_READ_ATTRIBUTES |
			DELETE | SYNCHRONIZE;
