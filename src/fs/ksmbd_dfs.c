				       ...
				       unsigned int *out_len)
{
	...
	int req_name_len;
	u16 version, dfs_rsp_server_type;
	...
	/*
	 * Early check: reject obviously too-small output buffers before
	 * spending time and memory on UTF-16 encoding allocations.
	 * The minimum response is the referral header plus the fixed-size
	 * referral entry (without variable-length path strings).
	 */
	fixed_size = dfs_referral_fixed_size(version);
	if (max_out_len < sizeof(struct resp_get_dfs_referral) + fixed_size) {
	...
	/* DFS-03: reject relative paths that do not start with a separator */
	if (request_path[0] != '\\' && request_path[0] != '/') {
		ksmbd_debug(SMB, "DFS: invalid request path (no leading backslash): %s\n",
			    request_path);
		kfree(request_path);
		rsp->hdr.Status = STATUS_INVALID_PARAMETER;
		return -EINVAL;
	...
	/*
	 * DFS-04: Determine whether this referral is for a DFS root
	 * (\\server\share) or a DFS link (\\server\share\subpath).
	 * Count path components: root has exactly 2 (server + share),
	 * link has more than 2.
	 */
	{
		...
		/* comp_count == 2 => \\server\share => root referral */
		dfs_rsp_server_type = (comp_count <= 2) ?
			DFS_SERVER_ROOT : DFS_SERVER_LINK;
	}
	...
	/*
	 * DFS-02: MS-DFSC §2.2.3 defines PathConsumed in bytes, NOT UTF-16
	 * codeunits.  req_name_len is already in bytes (from dfs_utf16_name_len),
	 * so use it directly without dividing by sizeof(__le16).
	 */
	dfs_rsp->path_consumed = cpu_to_le16(
		min_t(u32, req_name_len, U16_MAX));
	...
	if (version == DFS_REFERRAL_V4) {
		...
		ref4->server_type = cpu_to_le16(dfs_rsp_server_type);
		...
		ref3->server_type = cpu_to_le16(dfs_rsp_server_type);
		...
		/*
		 * MS-DFSC §2.2.5.2: For Version 2 referrals, Size MUST be
		 * the fixed header size only (excluding variable string data).
		 * For V3/V4, Size includes the full entry with strings.
		 */
		ref2->size = cpu_to_le16(sizeof(struct dfs_referral_level_2));
		ref2->server_type = cpu_to_le16(dfs_rsp_server_type);
