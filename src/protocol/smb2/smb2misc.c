...
VISIBLE_IF_KUNIT int check_smb2_hdr(struct smb2_hdr *hdr)
{
	/*
...
EXPORT_SYMBOL_IF_KUNIT(check_smb2_hdr);
 ...
 */
VISIBLE_IF_KUNIT int smb2_calc_size(void *buf, unsigned int *len)
{
...
EXPORT_SYMBOL_IF_KUNIT(smb2_calc_size);
...
VISIBLE_IF_KUNIT int smb2_validate_credit_charge(struct ksmbd_conn *conn,
						  struct smb2_hdr *hdr)
{
...
EXPORT_SYMBOL_IF_KUNIT(smb2_validate_credit_charge);
...
validate_credit:
	if (work->conn->vals->capabilities & SMB2_GLOBAL_CAP_LARGE_MTU) {
		int credit_rc = smb2_validate_credit_charge(work->conn, hdr);
		...
		/* TC-02: use >= to avoid wrap-around when outstanding_credits==UINT_MAX */
		if (work->conn->outstanding_credits >=
