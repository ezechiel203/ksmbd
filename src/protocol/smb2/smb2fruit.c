...
/*
 * Maximum allowed DataLength for an AAPL create context.  This caps the
 * amount of data the server will accept from the client, preventing
 * excessive memory allocation from a crafted request.  4 KiB is far
 * larger than any legitimate AAPL context payload.
 */
#define FRUIT_CREATE_CTX_MAX_DATA_LEN	4096
...
bool fruit_is_client_request(const void *buffer, size_t len)
{
	...
	if (le16_to_cpu(context->NameLength) != 4 ||
	    ...
	    le32_to_cpu(context->DataLength) > FRUIT_CREATE_CTX_MAX_DATA_LEN)
		return false;
...
static u64 fruit_compute_server_caps(void)
{
	...
	/*
	 * AAPL-02: kAAPL_SUPPORTS_READ_DIR_ATTR is NOT advertised.
	 *
	 * smb2_read_dir_attr_fill() computes rfork_len and FinderInfo but
	 * does not write them into the directory-entry wire format that macOS
	 * expects when this capability bit is set.  Advertising the bit while
	 * not implementing the protocol causes macOS Finder to interpret
	 * garbage data in directory listings.  Remove the bit until the wire
	 * packing is fully implemented.
	 */
...
int fruit_validate_create_context(const struct create_context *context)
{
	...
	if (le32_to_cpu(context->DataLength) > FRUIT_CREATE_CTX_MAX_DATA_LEN)
		return -EINVAL;
 ...
 */
int fruit_synthesize_afpinfo(struct mnt_idmap *idmap, struct dentry *dentry,
			     char *buf, size_t bufsize)
{
	ssize_t fi_len;
	__be32 val;

	if (!idmap || !dentry || !buf || bufsize < AFP_AFPINFO_SIZE)
		return -EINVAL;
	 ...
	 */
	fi_len = vfs_getxattr(idmap, dentry,
...
int ksmbd_fruit_read_afpinfo(struct path *path, void *buf, size_t len)
{
	...
	ret = fruit_synthesize_afpinfo(idmap, dentry, buf, len);
	if (ret > 0)
		return ret;
...
/*
 * ksmbd_fruit_write_afpinfo - Persist FinderInfo bytes from an AFP_AfpInfo write.
 *
 * AFP_AfpInfo structure (60 bytes, big-endian):
 *   bytes  0-15: signature + version (ignored on write)
 *   bytes 16-47: FinderInfo (32 bytes) → stored as com.apple.FinderInfo xattr
 *   bytes 48-59: ProDOS info (ignored on write)
 *
 * Returns 0 on success, negative errno on failure.
 */
int ksmbd_fruit_write_afpinfo(struct ksmbd_file *fp,
			      const char *buf, size_t count, loff_t offset)
{
	struct mnt_idmap *idmap;
	int rc;

	if (count < AFP_AFPINFO_SIZE || offset != 0)
		return 0;

	idmap = file_mnt_idmap(fp->filp);

	/*
	 * AAPL-04: Persist the full 60-byte AFP_AfpInfo structure in the
	 * DosStream xattr (XATTR_NAME_AFP_AFPINFO).  The read path
	 * (ksmbd_fruit_read_afpinfo) reads this xattr first, so write
	 * and read paths must use the same xattr for round-trip consistency.
	 */
	...
	/*
	 * Also store FinderInfo (bytes 16-47) as the native
	 * com.apple.FinderInfo xattr so that netatalk/AFP clients
	 * reading the same share see consistent icon/metadata.
	 */
	return ksmbd_vfs_setxattr(idmap,
				  &fp->filp->f_path,
				  APPLE_FINDER_INFO_XATTR_USER,
				  (void *)(buf + 16),
				  AFP_FINDER_INFO_SIZE,
				  0, true);
}
				  ...
				  struct path *entry_path)
{
	...
	if (test_share_config_flag(share,
		...
		rfork_len = vfs_getxattr(mnt_idmap(entry_path->mnt), dentry,
 ...
 * Advertises case sensitivity and full sync support.
 * kAAPL_SUPPORT_RESOLVE_ID is NOT advertised (no IOCTL handler exists).
 *
...
u64 ksmbd_fruit_get_volume_caps(struct ksmbd_share_config *share)
{
	...
	/*
	 * AAPL-03: kAAPL_SUPPORT_RESOLVE_ID is intentionally NOT advertised.
	 * The FSCTL_AAPL_RESOLVE_ID ioctl handler does not exist.  Advertising
	 * this bit causes macOS Finder alias resolution to fail silently when
	 * the server returns an error for the unhandled IOCTL.  Remove the
	 * cap until a proper handler is implemented.
	 */
			     ...
			     __le32 *ea_size_field)
{
	...
	/*
	 * Apple ReadDirAttr convention: overwrite the EaSize field with
	 * the UNIX mode (S_IFMT | permission bits) so that macOS Finder
	 * can display file-type icons without extra round-trips.
	 *
	 * This intentionally repurposes the EaSize field, which normally
	 * holds the extended-attribute size.  It is ONLY done when the
	 * connection has negotiated Apple/Fruit UNIX extensions
	 * (conn->is_fruit == true), so standard Windows clients are
	 * never affected.
	 *
	 * For special files (symlinks, char/block devices, fifos, sockets)
	 * the reparse tag is already set by the caller, so we must not
	 * overwrite it here.
	 */
	...
	if (test_share_config_flag(share,
		...
		rfork_len = vfs_getxattr(idmap, dentry,
					 XATTR_NAME_AFP_RESOURCE,
					 NULL, 0);
		if (rfork_len > 0)
			ksmbd_debug(SMB,
	...
	if (test_share_config_flag(share,
		...
		if (inode) {
			if (!inode_permission(idmap, inode, MAY_READ))
				access |= MAY_READ;
			if (!inode_permission(idmap, inode, MAY_WRITE))
				access |= MAY_WRITE;
			if (!inode_permission(idmap, inode, MAY_EXEC))
				access |= MAY_EXEC;
		}
