...
static struct smb_version_values smb311_server_values = {
	...
	/* Advertise server-to-client notification support (MS-SMB2 §2.2.4) */
	.capabilities = SMB2_GLOBAL_CAP_LARGE_MTU | SMB2_GLOBAL_CAP_NOTIFICATIONS,
	.max_read_size = SMB3_DEFAULT_IOSIZE,
	.max_write_size = SMB3_DEFAULT_IOSIZE,
	.max_trans_size = SMB3_DEFAULT_TRANS_SIZE,
	.max_credits = SMB2_MAX_CREDITS,
...
int init_smb3_11_server(struct ksmbd_conn *conn)
{
	...
	conn->max_cmds = ARRAY_SIZE(smb2_0_server_cmds);
	/*
	 * For SMB 3.1.1, the signing algorithm is negotiated via the
	 * SMB2_SIGNING_CAPABILITIES context which is decoded before this
	 * function runs.  Only set the default AES-CMAC if the client did
	 * not include a signing capabilities context.
	 */
	...
	/*
	 * B.5: MS-SMB2 §2.2.4 — SMB2_GLOBAL_CAP_ENCRYPTION (0x0040) should
	 * also be advertised for SMB 3.1.1 for compatibility with strict
	 * clients that check the capability bit before offering encryption
	 * contexts.  For 3.1.1 the actual cipher negotiation uses negotiate
	 * contexts, but some Windows clients check this bit first.
	 */
...
{
	if (sz < 1) {
		pr_warn("Invalid max credits value %u, setting to 1\n", sz);
		sz = 1;
	}
