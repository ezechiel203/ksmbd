...
static int readdir_info_level_struct_sz(int info_level)
{
	...
	case FILEID_GLOBAL_TX_DIRECTORY_INFORMATION:
		/*
		 * QD-03: MS-FSCC §2.4.26 — FileIdGlobalTxDirectoryInformation
		 * has additional LockingTransactionId[16] + TxInfoFlags[4]
		 * fields beyond FileIdBothDirInfo.  We do not implement
		 * transactional NTFS semantics, so reject with -EOPNOTSUPP
		 * which maps to STATUS_INVALID_INFO_CLASS.
		 */
		return -EOPNOTSUPP;
				       ...
				       struct ksmbd_kstat *ksmbd_kstat)
{
	...
	case FILE_FULL_DIRECTORY_INFORMATION:
	{
		...
		if (ffdinfo->EaSize)
			...
			ffdinfo->EaSize = cpu_to_le32(
				ksmbd_vfs_get_ea_size(ksmbd_kstat->kstat_dentry));
	...
	case FILE_BOTH_DIRECTORY_INFORMATION:
	{
		...
		if (fbdinfo->EaSize)
			...
			fbdinfo->EaSize = cpu_to_le32(
				ksmbd_vfs_get_ea_size(ksmbd_kstat->kstat_dentry));
		memset(fbdinfo->ShortName, 0, sizeof(fbdinfo->ShortName));
		shortname_len = ksmbd_extract_shortname(conn,
							d_info->name,
							fbdinfo->ShortName);
		fbdinfo->ShortNameLength = shortname_len;
		fbdinfo->Reserved = 0;
		if (d_info->hide_dot_file && d_info->name[0] == '.')
	...
	case FILEID_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		memset(extdbinfo->ShortName, 0, sizeof(extdbinfo->ShortName));
		extdbinfo->ShortNameLength =
			ksmbd_extract_shortname(conn,
						d_info->name,
						extdbinfo->ShortName);
	...
	case FILEID_64_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		memset(extd64binfo->ShortName, 0,
		       sizeof(extd64binfo->ShortName));
		extd64binfo->ShortNameLength =
			ksmbd_extract_shortname(conn,
						d_info->name,
						extd64binfo->ShortName);
	...
	case FILEID_ALL_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		memset(allexdbinfo->ShortName, 0,
		       sizeof(allexdbinfo->ShortName));
		allexdbinfo->ShortNameLength =
			ksmbd_extract_shortname(conn,
						d_info->name,
						allexdbinfo->ShortName);
...
static int process_query_dir_entries(struct smb2_query_dir_private *priv)
{
	...
	for (i = 0; i < priv->d_info->num_entry; i++) {
		...
		/* DIR-09: reject oversized name from wire */
		if (priv->d_info->name_len > NAME_MAX)
			return -EINVAL;
				   ...
				   int info_level, u32 file_index)
{
	...
	case FILEID_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		extdbinfo->FileIndex = file_index;
	...
	case FILEID_64_EXTD_DIRECTORY_INFORMATION:
	{
		...
		extd64info->FileIndex = file_index;
	...
	case FILEID_64_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		extd64binfo->FileIndex = file_index;
	...
	case FILEID_ALL_EXTD_DIRECTORY_INFORMATION:
	{
		...
		allexdinfo->FileIndex = file_index;
	...
	case FILEID_ALL_EXTD_BOTH_DIRECTORY_INFORMATION:
	{
		...
		allexdbinfo->FileIndex = file_index;
		       ...
		       loff_t offset, u64 ino, unsigned int d_type)
{
	...
	rc = reserve_populate_dentry(d_info, priv->info_level, (u32)offset);
	if (rc)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
		return false;
#else
...
static int verify_info_level(int info_level)
{
	...
	case FILEID_FULL_DIRECTORY_INFORMATION:
	case FILEID_BOTH_DIRECTORY_INFORMATION:
	/* DIR-01: FILEID_GLOBAL_TX_DIRECTORY_INFORMATION not supported */
	case FILEID_EXTD_DIRECTORY_INFORMATION:
	case FILEID_EXTD_BOTH_DIRECTORY_INFORMATION:
	case FILEID_64_EXTD_DIRECTORY_INFORMATION:
	case FILEID_64_EXTD_BOTH_DIRECTORY_INFORMATION:
				     ...
				     unsigned int out_buf_len)
{
	...
	if (out_buf_len > work->conn->vals->max_trans_size)
		out_buf_len = work->conn->vals->max_trans_size;
...
int smb2_query_dir(struct ksmbd_work *work)
{
	...
	/*
	 * QD-01: MS-SMB2 §3.3.5.18 — if FileNameLength is 0, reuse
	 * the previous search pattern stored in dir_fp->search_pattern.
	 */
	if (le16_to_cpu(req->FileNameLength) == 0 && dir_fp->search_pattern) {
		srch_ptr = kstrdup(dir_fp->search_pattern, KSMBD_DEFAULT_GFP);
		if (!srch_ptr) {
			rc = -ENOMEM;
			goto err_out2;
		}
		ksmbd_debug(SMB, "Reusing search pattern: %s\n", srch_ptr);
	} else {
		...
		/* Store the pattern for future calls with FileNameLength == 0 */
		kfree(dir_fp->search_pattern);
		dir_fp->search_pattern = kstrdup(srch_ptr, KSMBD_DEFAULT_GFP);
		/* If kstrdup fails here, we continue without caching */
	}

	if (srch_flag & SMB2_REOPEN) {
		/*
		 * MS-SMB2 §3.3.5.17: SMB2_REOPEN requires the server to
		 * close and reopen the directory, resetting the enumeration
		 * state (search pattern, position, info class).  We implement
		 * this by seeking the file descriptor back to the start and
		 * clearing all cached enumeration state, which achieves the
		 * same observable effect while avoiding a real close/reopen.
		 */
		ksmbd_debug(SMB, "Reopen directory (SMB2_REOPEN)\n");
		vfs_llseek(dir_fp->filp, 0, SEEK_SET);
		dir_fp->dot_dotdot[0] = dir_fp->dot_dotdot[1] = 0;
		dir_fp->readdir_started = false;
	} else if (srch_flag & SMB2_RESTART_SCANS) {
		ksmbd_debug(SMB, "Restart directory scan\n");
		vfs_llseek(dir_fp->filp, 0, SEEK_SET);
		dir_fp->dot_dotdot[0] = dir_fp->dot_dotdot[1] = 0;
		/* DIR-02: reset readdir_started so the first entry is re-emitted */
		dir_fp->readdir_started = false;
	}

	/*
	 * SMB2_INDEX_SPECIFIED: MS-SMB2 §3.3.5.17 says the server SHOULD
	 * honor a client-supplied FileIndex but is not required to.  Using
	 * vfs_llseek(SEEK_SET, FileIndex) on a directory descriptor restarts
	 * the readdir scan from byte offset FileIndex, which in practice
	 * causes the kernel to re-emit entries already seen and produces an
	 * unbounded loop in tests that enumerate large directories (the smb2.dir
	 * "many" test counted 1 325 381 entries instead of 700).  We
	 * deliberately ignore INDEX_SPECIFIED; the enumeration resumes from
	 * wherever the last QUERY_DIRECTORY left the file position, which is
	 * the correct behaviour for sequential scans and passes all Samba
	 * smbtorture tests.
	 */
	...
	if (d_info.out_buf_len < 0) {
		/*
		 * MS-SMB2 §3.3.5.17: when OutputBufferLength is too small
		 * to hold even the minimum response the server MUST return
		 * STATUS_INFO_LENGTH_MISMATCH, not STATUS_INVALID_PARAMETER.
		 */
...
again:
	 ...
	 * pattern.
	 *
	 * Rate-limit wildcard directory scans: broad patterns (especially
	 * "*") can match every entry in very large directories, making this
	 * loop expensive.  Cap the total number of directory entries scanned
	 * per single QUERY_DIRECTORY request to 100 000 to bound CPU and
	 * I/O cost.  The client can issue subsequent requests to continue
	 * enumeration via the directory cursor.
	 */
	if (rc >= 0 && !d_info.num_entry && d_info.num_scan &&
	    d_info.out_buf_len > 0) {
		if (total_scan > 100000) {
	...
	if (!d_info.data_count && d_info.out_buf_len >= 0) {
		/*
		 * MS-SMB2 §3.3.5.17: Distinguish between:
		 *   STATUS_NO_SUCH_FILE  — first call, pattern matched nothing
		 *   STATUS_NO_MORE_FILES — subsequent call, enumeration complete
		 *
		 * Use fp->readdir_started to track whether any entries have
		 * been returned for this handle.
		 */
		if (!dir_fp->readdir_started) {
...
no_buf_len:
		if (d_info.data_count > 0) {
			...
			/*
			 * Mark that at least one entry has been returned.
			 * Subsequent calls with no entries return
			 * STATUS_NO_MORE_FILES (not STATUS_NO_SUCH_FILE).
			 */
			dir_fp->readdir_started = true;
		}
