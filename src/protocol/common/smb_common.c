...
/*
 * next_dialect() - advance through a packed SMB1 dialect string list.
 * @dialect:  pointer to the start of the dialect buffer
 * @next_off: in/out: offset to advance by (updated to the length of current string)
 * @bcount:   remaining bytes in the dialect buffer
 *
 * SMB1 NEGOTIATE dialects are NUL-terminated ASCII strings packed
 * back-to-back, each preceded by a 0x02 byte (MS-SMB §2.2.4.52).
 * The caller strips the 0x02 prefix before passing the pointer here.
 * Returns NULL when the buffer is exhausted or the string is not NUL-terminated.
 */
...
static int ksmbd_negotiate_smb_dialect(void *buf)
{
	...
	if (proto == SMB1_PROTO_NUMBER) {
		...
		if (offsetof(struct smb_negotiate_req, DialectsArray) - RFC1002_HEADER_LEN +
...
/*
 * Characters invalid in 8.3 short names (MS-FSCC / MS-CIFS).
 * Space and characters above 0x7E are also stripped.
 */
static inline bool is_shortname_invalid_char(char c)
{
	static const char invalid[] = "\"*+,/:;<=>?[\\]|.";

	if ((unsigned char)c <= 0x20 || (unsigned char)c > 0x7E)
		return true;
	return strchr(invalid, c) != NULL;
}
 ...
 *
 * Generates an 8.3 short name from a long filename following the
 * Windows NTFS short name generation algorithm:
 *   - Strip invalid characters from the base name
 *   - Truncate base to at most 6 characters, append ~N suffix
 *   - N is derived from a checksum of the full long name
 *   - Extension is the first 3 valid characters after the last dot
 *   - All characters are uppercased
 *
 * Return:	shortname length in bytes (UTF-16) or 0 for '.' / '..'
 */
			    ...
			    char *shortname)
{
	...
	p = strrchr(longname, '.');
	if (p == longname) {
		/* name starts with a dot - use placeholder extension */
		strscpy(extension, "___", sizeof(extension));
	} else {
		if (p) {
			p++;
			while (*p && extlen < 3) {
				if (!is_shortname_invalid_char(*p))
	...
	/*
	 * Extract base name: up to 6 valid characters before the last dot.
	 * Strip invalid 8.3 characters and skip leading dots.
	 */
	p = longname;
	if (*p == '.') {
		p++;
	...
	/* Compute checksum over the entire long name for the ~N suffix */
	ptr = (const unsigned char *)longname;
	len = strlen(longname);
	for (; len > 0; len--, ptr++)
		csum += *ptr;

	/*
	 * Generate a 1-digit suffix (1-9) derived from the checksum.
	 * This approximates Windows behaviour where ~1 is the first
	 * collision, ~2 the second, etc.  Without a directory scan we
	 * use the checksum to pick a deterministic digit.
	 */
	suffix_digit = (csum % 9) + 1;

	/* Build the 8.3 name: BASE~N.EXT */
	memcpy(out, base, baselen);
	out[baselen] = MAGIC_CHAR;
	out[baselen + 1] = '0' + suffix_digit;

	if (dot_present) {
		out[baselen + 2] = PERIOD;
		memcpy(out + baselen + 3, extension, extlen + 1);
	} else {
		out[baselen + 2] = '\0';
	}
...
static int smb_handle_negotiate(struct ksmbd_work *work)
{
	...
	if (ksmbd_iov_pin_rsp(work, (void *)neg_rsp + RFC1002_HEADER_LEN,
			      sizeof(struct smb_negotiate_unsupported_rsp) - RFC1002_HEADER_LEN))
		return -ENOMEM;
...
int ksmbd_smb_check_shared_mode(struct file *filp, struct ksmbd_file *curr_fp)
{
	...
	list_for_each_entry(prev_fp, &curr_fp->f_ci->m_fp_list, node) {
		/*
		 * Safety limit: if the list has more entries than is
		 * plausible (e.g. corrupted cycle), bail out to avoid
		 * spinning forever.  Also check for conn shutdown to
		 * allow kworkers to exit promptly during teardown.
		 */
		if (unlikely(++loop_count > 65536)) {
			pr_warn_ratelimited("ksmbd: m_fp_list loop limit hit (%u), possible list corruption\n",
					    loop_count);
			rc = -EBUSY;
			break;
		}
		...
		struct ksmbd_share_config *share)
{
	...
	/*
	 * If fsids are already overridden (e.g., smb2_set_info already
	 * called ksmbd_override_fsids before dispatching to a VFS helper
	 * that also calls it), just bump the depth counter and return.
	 * The matching ksmbd_revert_fsids will decrement without actually
	 * reverting credentials, preventing the BUG at kernel/cred.c:104.
	 */
	if (work->saved_cred) {
		work->saved_cred_depth++;
		return 0;
	}
...
void ksmbd_revert_fsids(struct ksmbd_work *work)
{
	...
	/*
	 * If we are in a nested override (depth > 0), just decrement
	 * the counter without actually reverting credentials.  The
	 * outermost caller will do the real revert.
	 */
	if (work->saved_cred_depth > 0) {
		work->saved_cred_depth--;
		return;
	}
