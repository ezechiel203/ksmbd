 ...
 * FORTIFY_SOURCE sub-object tracking produces false-positive warnings for
 * these intentional writes.  This barrier hides pointer provenance from the
 * compiler so __builtin_object_size cannot determine a sub-object bound.
 */
static __always_inline void *__smb1_unfence(void *ptr)
{
...
int smb_allocate_rsp_buf(struct ksmbd_work *work)
{
	...
	if (cmd == SMB_COM_TRANSACTION || cmd == SMB_COM_NT_TRANSACT)
		sz = large_sz;
 ...
 * @buf:	server response buffer (server-controlled, not client input)
 * @buf_size:	total allocated size of @buf
 * @min_size:	minimum bytes required for the subcommand response
 *
 * Return:      pointer to andx subcommand response area, or NULL if the
 *              buffer has insufficient space.
 */
static char *andx_response_buffer(char *buf, size_t buf_size, size_t min_size)
{
	int pdu_length = get_rfc1002_len(buf);
	size_t offset = 4 + (size_t)pdu_length;

	if (offset + min_size > buf_size) {
		pr_err_ratelimited("SMB1 AndX response buffer too small (need %zu, have %zu)\n",
				   offset + min_size, buf_size);
		return NULL;
	}
	return buf + offset;
}
...
int smb_session_disconnect(struct ksmbd_work *work)
{
	...
	/* P1.7: response must be WordCount=2 (AndX block) + ByteCount=0.
	 * smb_com_lock_rsp has exactly this layout (AndX + ByteCount).
	 */
	struct smb_com_lock_rsp *rsp = work->response_buf;

	/* P1.6: MS-SMB §2.2.4.54 — only invalidate the specific UID's
	 * session, do NOT tear down the entire TCP connection.
	 * Only call ksmbd_conn_set_exiting() when no sessions remain.
	 */
	...
	/* P1.7: Build LOGOFF_ANDX response: WordCount=2, AndXCommand=0xFF */
	rsp->hdr.WordCount = 2;
	rsp->AndXCommand = SMB_NO_MORE_ANDX_COMMAND;
	rsp->AndXReserved = 0;
	rsp->AndXOffset = 0;
	rsp->ByteCount = 0;
	inc_rfc1001_len(&rsp->hdr, 2 * 2 + 2);
...
int smb_tree_disconnect(struct ksmbd_work *work)
{
	...
	if (!tcon) {
		pr_err("Invalid tid %d\n", req_hdr->Tid);
		/* P1.8: MS-SMB §2.2.4.51 requires STATUS_SMB_BAD_TID here */
			     ...
			     struct smb_com_tconx_rsp_ext *rsp)
{
	...
	if (test_share_config_flag(share, KSMBD_SHARE_FLAG_PIPE)) {
		...
		memcpy(__smb1_unfence(buf), SERVICE_IPC_SHARE, length);
		...
		*buf = '\0';
		...
		memcpy(__smb1_unfence(buf), SERVICE_DISK_SHARE, length);
		buf[length] = '\0';
		length += 1;
...
int smb_tree_connect_andx(struct ksmbd_work *work)
{
	...
	/* P1.24: If DISCONNECT_TID flag is set, disconnect the current TID */
	if (le16_to_cpu(req->Flags) & DISCONNECT_TID) {
		struct ksmbd_tree_connect *old_tcon = work->tcon;

		if (old_tcon) {
			ksmbd_close_tree_conn_fds(work);
			write_lock(&sess->tree_conns_lock);
			if (old_tcon->t_state != TREE_DISCONNECTED) {
				WARN_ON_ONCE(refcount_dec_and_test(&old_tcon->refcount));
				old_tcon->t_state = TREE_DISCONNECTED;
			}
			write_unlock(&sess->tree_conns_lock);
			ksmbd_tree_conn_disconnect(sess, old_tcon);
			work->tcon = NULL;
		}
	}
	...
	{
		unsigned int pw_len = le16_to_cpu(req->PasswordLength);
		unsigned int bc = le16_to_cpu(req->ByteCount);

		if (pw_len > bc) {
			pr_err("SMB1 tree connect: PasswordLength %u exceeds ByteCount %u\n",
			       pw_len, bc);
		...
		if (!pw_len) {
			...
			treename = smb_strndup_from_utf16(req->Password + pw_len,
							  256, true, conn->local_nls);
		}
	}
...
int smb_handle_negotiate(struct ksmbd_work *work)
{
	...
	if (conn->dialect == BAD_PROT_ID) {
		/* P1.9: MS-SMB §2.2.4.52.2 — when no common dialect exists,
		 * set DialectIndex=0xFFFF with STATUS_SUCCESS (WordCount=1).
		 * Do NOT return STATUS_INVALID_LOGON_TYPE.
		 */
		neg_rsp->hdr.WordCount = 1;
		neg_rsp->DialectIndex = cpu_to_le16(0xFFFF);
		neg_rsp->hdr.Status.CifsError = STATUS_SUCCESS;
		/* Write ByteCount=0 after the single word parameter */
		*(__le16 *)((char *)neg_rsp + sizeof(struct smb_hdr) + 2) =
			cpu_to_le16(0);
		inc_rfc1001_len(neg_rsp, 1 * 2 + 2);
	...
	/* P1.10: MaxRawSize MUST be 0 when CAP_RAW_MODE is not advertised
	 * (MS-SMB §2.2.4.52.2).
	 */
	neg_rsp->MaxRawSize = (SMB1_SERVER_CAPS & CAP_RAW_MODE) ?
		cpu_to_le32(SMB1_MAX_RAW_SIZE) : 0;
	...
	if (conn->use_spnego == false) {
		/* P1.21: non-extended-security path: append workgroup OEM
		 * NUL-terminated string after 8-byte encryption key.
		 */
		char *domain_ptr;
		int domain_len;
		...
		/* Append NUL-terminated OEM workgroup name after challenge */
		domain_ptr = (char *)neg_rsp->u.EncryptionKey +
			CIFS_CRYPTO_KEY_SIZE;
		domain_len = strlen(ksmbd_work_group());
		memcpy(__smb1_unfence(domain_ptr), ksmbd_work_group(),
		       domain_len + 1);
		neg_rsp->ByteCount = cpu_to_le16(CIFS_CRYPTO_KEY_SIZE +
						 domain_len + 1);
		/* Adjust pdu length: 17 words + key + domain + NUL */
		inc_rfc1001_len(neg_rsp,
				(17 * 2 + CIFS_CRYPTO_KEY_SIZE +
				 domain_len + 1));
	} else {
				   ...
				   struct smb_com_session_setup_old_resp *rsp)
{
	...
	{
		unsigned int ci_len = le16_to_cpu(req->CaseInsensitivePasswordLength);
		unsigned int cs_len = le16_to_cpu(req->CaseSensitivePasswordLength);
		unsigned int pdu_bc = le16_to_cpu(req->ByteCount);

		if (check_add_overflow(ci_len, cs_len, (unsigned int *)&offset) ||
		    offset > pdu_bc) {
			pr_err("SMB1 session setup: password lengths exceed ByteCount\n");
			err = -EINVAL;
			goto out_err;
		}
	}
	...
	/*
	 * SMB1-13: Validate password lengths for the non-extended-security path.
	 * CaseSensitivePasswordLength must be 0 (no password), 24 (NTLMv1), or
	 * >= CIFS_ENCPWD_SIZE + 28 (NTLMv2 blob minimum).
	 * Reject any other values to avoid underflow in NTLMv2 length math.
	 */
	{
		unsigned int cs_len = le16_to_cpu(req->CaseSensitivePasswordLength);

		if (cs_len != 0 && cs_len != CIFS_AUTH_RESP_SIZE &&
		    cs_len < (CIFS_ENCPWD_SIZE + 28)) {
			pr_err("SMB1 session setup: invalid CaseSensitivePasswordLength %u\n",
			       cs_len);
			err = -EINVAL;
			goto out_err;
		}
...
no_password_check:
	...
	/*
	 * S-08: Cap workgroup/OS/LanMan string writes to avoid overflowing
	 * the response buffer.  Each string is converted to UTF-16 into a
	 * local 64-byte buffer (str[32] = 32 __le16 = 64 bytes).
	 */
	...
	/* P1.23: use configured workgroup name instead of hardcoded value */
	{
		const char *wg = ksmbd_work_group();
		size_t wg_len = strlen(wg);

		/* Clamp workgroup name to fit in str[] buffer */
		if (wg_len > 31)
			wg_len = 31;
		len = smb_strtoUTF16(str, wg, wg_len, conn->local_nls);
	}
	len = UNICODE_LEN(len + 1);
	if (len > sizeof(str))
		len = sizeof(str);
				 ...
				 struct smb_com_session_setup_resp *rsp)
{
	...
	if (negblob->MessageType == NtLmNegotiate) {
		...
		memset(__smb1_unfence(chgblob), 0, sizeof(struct challenge_message));

		if (conn->use_spnego) {
			int sz;
			/* AUTH-06: account for workgroup name which may be longer */
			{
			...
			/*
			 * SMB1-23: ANDX variable-length response bounds check.
			 *
			 * SecurityBlob is written past the fixed response header.
			 * Guard against an unexpectedly large SPNEGO blob that
			 * would overflow the response buffer.  SMB1 responses
			 * are capped at SMB1_MAX_PAYLOAD; 64 KiB is a safe
			 * upper bound for the SPNEGO blob.
			 */
			if (spnego_blob_len > 65535) {
				pr_err_ratelimited("SMB1: SPNEGO blob too large (%u bytes)\n",
						   spnego_blob_len);
				kfree(spnego_blob);
				kfree(neg_blob);
				err = -EINVAL;
			...
			unsigned int max_blob_sz;
			{
				...
				max_blob_sz = (unsigned int)(sizeof(struct negotiate_message) +
					(max_name_len * 2 + 1 + 4) * 6);
			}
			neg_blob_len = ksmbd_build_ntlmssp_challenge_blob(
					chgblob,
					max_blob_sz,
		...
		/* P1.11: when mechToken was extracted by SPNEGO decoder, use
		 * conn->mechTokenLen (the inner NTLMSSP token size) rather
		 * than the outer SecurityBlobLength to avoid reading beyond
		 * the actual NTLMSSP blob.
		 */
...
no_password_check:
	...
	/* P1.22: Append NativeOS, NativeLanMan, PrimaryDomain as
	 * UTF-16LE NUL-terminated strings after the security blob.
	 * Required by MS-SMB §2.2.4.53.2 for the extended security path.
	 * Only append on final success response (not PROCESSING_REQUIRED).
	 */
	if (!err && rsp->hdr.Status.CifsError == STATUS_SUCCESS) {
		__le16 str22[32];
		int str_offset = 1; /* 1-byte pad for alignment */
		int slen;
		char *str_base = (char *)rsp->SecurityBlob +
				 le16_to_cpu(rsp->SecurityBlobLength);

		slen = smb_strtoUTF16(str22, "Linux", 5, conn->local_nls);
		slen = UNICODE_LEN(slen + 1);
		memcpy(__smb1_unfence(str_base + str_offset), str22, slen);
		str_offset += slen;

		slen = smb_strtoUTF16(str22, "ksmbd", 5, conn->local_nls);
		slen = UNICODE_LEN(slen + 1);
		memcpy(__smb1_unfence(str_base + str_offset), str22, slen);
		str_offset += slen;

		slen = smb_strtoUTF16(str22, ksmbd_work_group(),
				      strlen(ksmbd_work_group()),
				      conn->local_nls);
		slen = UNICODE_LEN(slen + 1);
		memcpy(__smb1_unfence(str_base + str_offset), str22, slen);
		str_offset += slen;
		...
		rsp->ByteCount = cpu_to_le16(
			le16_to_cpu(rsp->SecurityBlobLength) + str_offset);
		inc_rfc1001_len(rsp, str_offset);
	}
...
int smb_session_setup_andx(struct ksmbd_work *work)
{
	...
	if (uid != 0) {
		...
		/* P1.26: VcNumber == 0 means client is establishing the first
		 * virtual circuit; tear down any pre-existing sessions on this
		 * connection per MS-SMB §3.3.5.3.
		 */
		__le16 vc_number = (pSMB->req.hdr.WordCount == 12) ?
			pSMB->req.VcNumber :
			pSMB->req_no_secext.VcNumber;

		if (le16_to_cpu(vc_number) == 0) {
			struct ksmbd_session *old_sess;
			unsigned long sess_id;

			ksmbd_debug(SMB,
				    "VcNumber=0: tearing down existing sessions\n");
			/*
			 * Iterate all sessions on this connection and log off
			 * each one.  We must drop conn_lock while calling
			 * ksmbd_tree_conn_session_logoff() since it acquires
			 * tree_conns_lock internally.
			 */
			xa_for_each(&conn->sessions, sess_id, old_sess) {
				ksmbd_tree_conn_session_logoff(old_sess);
				xa_erase(&conn->sessions, old_sess->id);
				hash_del(&old_sess->hlist);
				ksmbd_session_destroy(old_sess);
			}
		}
...
int smb_locking_andx(struct ksmbd_work *work)
{
	...
	/* P1.17: cast timeout to s32 so that 0xFFFFFFFF = -1 = infinite wait,
	 * 0 = non-blocking, positive = milliseconds wait timeout.
	 * SMB1-11: cap to 30 seconds to prevent indefinitely blocking a worker
	 * thread. A full async conversion is the proper fix but too invasive
	 * for P1.
	 */
	...
	/*
	 * P1.16: Per MS-SMB §2.2.4.32.1, the Locks buffer contains:
	 *   Unlocks[NumberOfUnlocks] first, then Locks[NumberOfLocks].
	 * Fix buffer pointer arithmetic accordingly.
	 */
	...
	if (req->LockType & LOCKING_ANDX_CHANGE_LOCKTYPE) {
		/* P1.15: CHANGE_LOCKTYPE is not supported; return
		 * STATUS_NOT_SUPPORTED as required by MS-SMB spec.
		 */
	...
	if (req->LockType & LOCKING_ANDX_CANCEL_LOCK) {
		/*
		 * SMB1-001: CANCEL_LOCK — cancel a pending (blocked) lock
		 * matching the range specified in the first lock element.
		 */
		unsigned long long cancel_offset = 0, cancel_length = 0;
		struct ksmbd_lock *cancel_lock;

		if (req->LockType & LOCKING_ANDX_LARGE_FILES) {
			if (lock_count > 0) {
				cancel_offset = (unsigned long long)le32_to_cpu(
					lock_ele64[0].OffsetLow);
				cancel_length = (unsigned long long)le32_to_cpu(
					lock_ele64[0].LengthLow);
				cancel_offset |= (unsigned long long)le32_to_cpu(
					lock_ele64[0].OffsetHigh) << 32;
				cancel_length |= (unsigned long long)le32_to_cpu(
					lock_ele64[0].LengthHigh) << 32;
			}
		} else {
			if (lock_count > 0) {
				cancel_offset = (unsigned long long)le32_to_cpu(
		...
		list_for_each_entry(cancel_lock, &work->conn->lock_list, clist) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
			if (file_inode(cancel_lock->fl->c.flc_file) !=
			    file_inode(filp))
#else
			if (file_inode(cancel_lock->fl->fl_file) !=
			    file_inode(filp))
#endif
				continue;
			if (cancel_lock->start == cancel_offset &&
			    cancel_lock->end == cancel_offset + cancel_length) {
				ksmbd_vfs_posix_lock_unblock(cancel_lock->fl);
				break;
			}
		}
	...
	/*
	 * P1.16: Per MS-SMB §3.3.5.15, process unlocks BEFORE locks.
	 */
	for (i = 0; i < unlock_count; i++) {
		...
		/* fl_end is inclusive: fl_start + length - 1 (SMB1-006) */
		if (length == 0)
			flock->fl_end = OFFSET_MAX;
		else if (offset + length - 1 > OFFSET_MAX)
			flock->fl_end = OFFSET_MAX;
		else
			flock->fl_end = offset + length - 1;
		...
		hlist_for_each_entry(conn, &conn_hash[bkt].head,
			...
			list_for_each_entry_safe(cmp_lock, tmp2, &conn->lock_list, clist) {
				...
				if (smb_lock->zero_len &&
					...
					same_zero_lock = 1;
					/* unlock both before jumping out */
					spin_unlock(&conn->llist_lock);
					spin_unlock(&conn_hash[bkt].lock);
					goto out_check_cl;
				}
...
int smb_trans(struct ksmbd_work *work)
{
	...
	if (!strncmp(pipe, "LANMAN", sizeof("LANMAN"))) {
		/*
		 * SMB1-19: LANMAN RAP ParameterCount validation.
		 *
		 * The RAP parameter buffer begins with a 2-byte FunctionCode.
		 * Without at least 2 bytes, any access to the function code
		 * or subsequent fields is an OOB read.  NetServerEnum2 (0x68)
		 * requires at least 10 bytes.  Enforce the minimum here before
		 * passing the buffer to the RPC layer.
		 */
		if (param_cnt < 2) {
			ksmbd_debug(SMB, "LANMAN RAP: ParameterCount %u too small\n",
				    param_cnt);
			rsp->hdr.Status.CifsError = STATUS_INVALID_PARAMETER;
			ret = -EINVAL;
			goto out;
...
int smb_nt_create_andx(struct ksmbd_work *work)
{
	...
	if (req->RootDirectoryFid) {
		...
		if (fp)
			...
			memset(__smb1_unfence(&rsp->hdr.WordCount), 0, 3);
			return -EINVAL;
		}
		ksmbd_fd_put(work, fp);
	}
	 ...
	 */
	{
		...
		unsigned int pdu_len = get_rfc1002_len(work->request_buf) + 4;
		unsigned int fn_off = offsetof(struct smb_com_open_req, fileName);
		...
		max_name_len = pdu_len - fn_off;

		/* Unicode paths have a 1-byte pad before fileName */
		if (is_smbreq_unicode(&req->hdr)) {
			if (max_name_len < 1 || name_len > max_name_len - 1) {
				rsp->hdr.Status.CifsError =
					STATUS_INVALID_PARAMETER;
				return -EINVAL;
			}
		...
		src = kzalloc(name_len + 2, KSMBD_DEFAULT_GFP);
		if (!src) {
			rsp->hdr.Status.CifsError = STATUS_NO_MEMORY;
			return -ENOMEM;
		}
		...
		if (is_smbreq_unicode(&req->hdr)) {
			memcpy(src, req->fileName + 1, name_len);
			is_unicode = true;
		} else {
			memcpy(src, req->fileName, name_len);
			is_unicode = false;
		}
	}
	...
	if (extended_reply) {
		...
		/* A-02: GuestAccess = 0 for authenticated users (MS-SMB §2.2.4.9.2) */
		ext_rsp->GuestAccess = 0;
		ext_rsp->ByteCount = 0;
...
static int smb_read_andx_pipe(struct ksmbd_work *work)
{
	...
	rsp->Remaining = 0;
	/* P1.3: remove duplicate DataCompactionMode assignment */
...
int smb_read_andx(struct ksmbd_work *work)
{
	...
	rsp->ByteCount = cpu_to_le16(nbytes);
	inc_rfc1001_len(&rsp->hdr, (rsp->hdr.WordCount * 2));
	work->resp_hdr_sz = get_rfc1002_len(rsp) + RFC1002_HEADER_LEN;
...
int smb_write_andx(struct ksmbd_work *work)
{
	...
	/* P1.1: MS-SMB §2.2.4.32.1 — bit 0 is write-through, not full word */
...
int smb_echo(struct ksmbd_work *work)
{
	...
	/*
	 * Clamp data_count to actual PDU payload to prevent OOB read.
	 * The request buffer may be smaller than ByteCount claims.
	 */
	{
		unsigned int pdu_len = get_rfc1002_len(req);
		unsigned int hdr_off = offsetof(struct smb_com_echo_req, Data) -
				       RFC1002_HEADER_LEN;
		unsigned int max_data = (pdu_len > hdr_off) ? pdu_len - hdr_off : 0;
...
static int smb_get_acl(struct ksmbd_work *work, struct path *path)
{
	...
	aclbuf = (struct cifs_posix_acl *)(work->response_buf +
			sizeof(struct smb_com_trans2_rsp) + RFC1002_HEADER_LEN);
...
static int smb_readlink(struct ksmbd_work *work, struct path *path)
{
	...
	/*
	 * SMB1-24: Absolute symlink target path disclosure prevention.
	 *
	 * If the symlink target is an absolute path, verify that it resolves
	 * to a location within the share root.  Returning a verbatim kernel
	 * absolute path (e.g. "/etc/passwd") to the client leaks server
	 * filesystem layout and may allow unauthorized access.
	 *
	 * If the target path does not resolve within the share, deny access.
	 * Relative symlinks are permitted unconditionally.
	 */
	if (buf[0] == '/' && work->tcon && work->tcon->share_conf) {
		struct path link_path;
		int lookup_err;

		lookup_err = kern_path(buf, LOOKUP_FOLLOW, &link_path);
		if (lookup_err == 0) {
			bool within_share = path_is_under(&link_path,
					&work->tcon->share_conf->vfs_path);
			path_put(&link_path);
			if (!within_share) {
				pr_warn_ratelimited("SMB1: blocked absolute symlink outside share: %s\n",
						    buf);
				rsp->hdr.Status.CifsError = STATUS_ACCESS_DENIED;
				err = -EACCES;
				goto out;
		...
		/* If kern_path fails the target does not exist — still return
		 * it for compatibility; the client will get ENOENT on access. */
	}
...
static int smb_get_ea(struct ksmbd_work *work, struct path *path)
{
	...
	buf_free_len = work->response_sz - (get_rfc1002_len(rsp) + RFC1002_HEADER_LEN) -
...
static int query_fs_info(struct ksmbd_work *work)
{
	...
	{
		/* A.2: SMB_INFO_VOLUME (level 2) — OEM volume label.
		 * Response: SerialNumber (4) + VolumeLabelSize (1) + VolumeLabel (OEM)
		 */
		char *vbuf = (char *)(&rsp->Pad + 1);
		int vlabel_len;

		ksmbd_debug(SMB, "GOT SMB_INFO_VOLUME\n");
		*(__le32 *)vbuf = cpu_to_le32(0xbc3ac512); /* serial */
		vlabel_len = strlen(share->name);
		if (vlabel_len > 11)
			vlabel_len = 11;
		vbuf[4] = (char)vlabel_len;
		memcpy(vbuf + 5, share->name, vlabel_len);
		rsp->t2.TotalDataCount = cpu_to_le16(5 + vlabel_len);
		break;
	}
	case SMB_INFO_IS_NAME_VALID:
		/* A.2: SMB_INFO_IS_NAME_VALID (level 6) — always succeed */
		ksmbd_debug(SMB, "GOT SMB_INFO_IS_NAME_VALID\n");
		rsp->t2.TotalDataCount = 0;
		break;
	...
	case SMB_QUERY_FS_ATTRIBUTE_INFO:
	{
		...
		{
			__le16 *fsname = (__le16 *)((char *)info + 12);
			int fsname_chars = smb_strtoUTF16(fsname, "NTFS", 4, conn->local_nls);
			int fsname_bytes = fsname_chars * 2;

			info->FileSystemNameLen = cpu_to_le32(fsname_bytes);
			rsp->t2.TotalDataCount = cpu_to_le16(12 + fsname_bytes);
		}
		break;
	}
				      ...
				      struct ksmbd_kstat *ksmbd_kstat)
{
	...
	case SMB_FIND_FILE_BOTH_DIRECTORY_INFO:
	{
		...
		fbdinfo->EaSize = 0;
		fbdinfo->Reserved = 0;
		memset(fbdinfo->ShortName, 0, sizeof(fbdinfo->ShortName));
		short_name_len = ksmbd_extract_shortname(conn,
							 d_info->name,
							 fbdinfo->ShortName);
		fbdinfo->ShortNameLength = short_name_len;
	...
	case SMB_FIND_FILE_ID_BOTH_DIR_INFO:
	{
		...
		short_name_len = ksmbd_extract_shortname(conn,
							 d_info->name,
							 fibdinfo->ShortName);
		fibdinfo->ShortNameLength = short_name_len;
...
static int find_first(struct ksmbd_work *work)
{
	...
	rsp->t2.ParameterOffset =
		cpu_to_le16(sizeof(struct smb_com_trans2_rsp) - RFC1002_HEADER_LEN);
	rsp->t2.ParameterDisplacement = 0;
...
static int query_file_info_pipe(struct ksmbd_work *work)
{
	...
	/* P1.12: second field is Directory, not a duplicate DeletePending */
...
static int query_file_info(struct ksmbd_work *work)
{
	...
	{
		/*
		 * A.2: Return a single "::$DATA" stream entry for non-alternate
		 * stream files.  Format: smb2_file_stream_info (same layout as
		 * SMB1 FILE_STREAM_INFORMATION).
		 */
		struct smb2_file_stream_info *sinfo;
		static const __le16 default_stream_name[] = {
			/* "::$DATA" in UTF-16LE */
			cpu_to_le16(':'), cpu_to_le16(':'),
			cpu_to_le16('$'), cpu_to_le16('D'),
			cpu_to_le16('A'), cpu_to_le16('T'),
			cpu_to_le16('A'),
		};
		int sname_bytes = sizeof(default_stream_name);
		int total_bytes = sizeof(*sinfo) + sname_bytes;
		__u16 bc;

		ksmbd_debug(SMB, "SMB_QUERY_FILE_STREAM_INFO\n");
		rsp_hdr->WordCount = 10;
		rsp->t2.TotalParameterCount = cpu_to_le16(2);
		rsp->t2.TotalDataCount = cpu_to_le16(total_bytes);
		rsp->t2.Reserved = 0;
		rsp->t2.ParameterCount = cpu_to_le16(2);
		rsp->t2.ParameterOffset = cpu_to_le16(56);
		rsp->t2.ParameterDisplacement = 0;
		rsp->t2.DataCount = cpu_to_le16(total_bytes);
		rsp->t2.DataOffset = cpu_to_le16(60);
		rsp->t2.DataDisplacement = 0;
		rsp->t2.SetupCount = 0;
		rsp->t2.Reserved1 = 0;
		bc = 2 + 1 + total_bytes; /* param_count + pad1 + data */
		rsp->ByteCount = cpu_to_le16(bc);
		rsp->Pad = 0;
		ptr = (char *)&rsp->Pad + 1;
		memset(__smb1_unfence(ptr), 0, 4); /* 2-byte param + 2 pad */
		sinfo = (struct smb2_file_stream_info *)(ptr + 4);
		sinfo->NextEntryOffset = 0;
		sinfo->StreamNameLength = cpu_to_le32(sname_bytes);
		sinfo->StreamSize = cpu_to_le64(st.size);
		sinfo->StreamAllocationSize = cpu_to_le64(st.blocks << 9);
		memcpy(sinfo->StreamName, default_stream_name, sname_bytes);
		inc_rfc1001_len(rsp_hdr, 10 * 2 + bc);
		break;
	}
	default:
		pr_err("query path info not implemnted for %x\n",
...
static int smb_fileinfo_rename(struct ksmbd_work *work)
{
	...
	/*
	 * P1.13: Pass RENAME_NOREPLACE when overwrite flag is clear so that
	 * the destination is not silently replaced.  When overwrite is set,
	 * the VFS rename (flags=0) will atomically replace the destination —
	 * do NOT truncate the source file.
	 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 4, 0)
	rc = ksmbd_vfs_rename(work, &fp->filp->f_path, newname,
			      info->overwrite ? 0 : RENAME_NOREPLACE);
#else
	rc = ksmbd_vfs_fp_rename(work, fp, newname);
#endif
	if (rc) {
...
int smb_trans2(struct ksmbd_work *work)
{
	...
	unsigned int param_off, param_cnt, data_off, data_cnt;
	...
	/* A.3: Bounds check ParameterOffset and DataOffset before any
	 * sub-handler accesses them (MS-SMB §5.4 security requirement).
	 * Offsets are relative to &hdr.Protocol so subtract 4 from our
	 * req_buf_len which starts at the NetBIOS header.
	 */
	param_off = le16_to_cpu(req->ParameterOffset);
	param_cnt = le16_to_cpu(req->ParameterCount);
	data_off  = le16_to_cpu(req->DataOffset);
	...
	if (param_off > req_buf_len || param_cnt > req_buf_len - param_off ||
	    data_off  > req_buf_len || data_cnt  > req_buf_len - data_off) {
		pr_err("TRANS2: invalid offsets/counts P@%u/%u D@%u/%u len=%u\n",
		       param_off, param_cnt, data_off, data_cnt, req_buf_len);
		rsp_hdr->Status.CifsError = STATUS_INVALID_PARAMETER;
		return -EINVAL;
	}
...
/* Full NT_TRANSACT dispatcher is defined below after subcommand handlers */
...
int smb_query_info(struct ksmbd_work *work)
{
	...
	if (S_ISDIR(st.mode))
		attr |= ATTR_DIRECTORY;
	/* P1.19: regular files should have ATTR_ARCHIVE set per MS-SMB */
	if (S_ISREG(st.mode))
		attr |= ATTR_ARCHIVE;
...
int smb_open_andx(struct ksmbd_work *work)
{
	...
	rsp->Fid = fp->volatile_id;
	/* P1.4: use actual DOS attributes from stat, not hardcoded ATTR_NORMAL */
...
int smb_setattr(struct ksmbd_work *work)
{
	...
	/* P1.14: MS-SMB SETATTR LastWriteTime semantics:
	 * 0 = do not change (skip),
	 * 0xFFFFFFFF = set to current server time,
	 * otherwise = convert DOS time and set.
	 */
	{
		__u32 lwt = le32_to_cpu(req->LastWriteTime);

		if (lwt == 0) {
			/* 0 means do not change last write time */
		} else if (lwt == 0xFFFFFFFF) {
			/* 0xFFFFFFFF means current server time */
			ktime_get_real_ts64(&attrs.ia_mtime);
			attrs.ia_valid |= (ATTR_MTIME | ATTR_MTIME_SET);
		} else {
			attrs.ia_mtime.tv_sec = lwt;
			attrs.ia_mtime.tv_nsec = 0;
			attrs.ia_valid |= (ATTR_MTIME | ATTR_MTIME_SET);
		}
	}
...
bool smb1_is_sign_req(struct ksmbd_work *work, unsigned int command)
{
	...
	/*
	 * Only skip signing verification for negotiate (no session yet)
	 * and the first session setup (session key not yet established).
	 * All other commands including tree connect must be verified.
	 */
...
/*
 * dos_date_time_to_unix() - convert SMB1 DOS date/time fields to unix time
 ...
 * DOS time format: bits 15-11 = hour, bits 10-5 = minute, bits 4-0 = sec/2
 *
 * Returns unix timestamp (seconds since epoch), or 0 on invalid input.
 */
static time64_t dos_date_time_to_unix(__le16 date, __le16 time)
{
	...
	tm.tm_mon  = ((d >> 5) & 0x0F) - 1;  /* 0-based month */
	tm.tm_mday = (d & 0x1F);
...
/**
 * smb_query_information2() - QUERY_INFORMATION2 handler
 * @work:	smb work containing request
 *
 * Returns file timestamps and size using the SMB1 DOS date/time format.
 *
 * Return:	0 on success, otherwise error
 */
int smb_query_information2(struct ksmbd_work *work)
{
	...
	fp = ksmbd_lookup_fd_fast(work, req->Fid);
	if (!fp) {
		pr_err("QUERY_INFORMATION2: invalid Fid %u\n", req->Fid);
		rsp->hdr.Status.CifsError = STATUS_INVALID_HANDLE;
		return -ENOENT;
	}
	...
	/* Creation date/time — use ctime as creation approximation */
	unix_to_dos_time(st.ctime, &rsp->CreateTime, &rsp->CreateDate);
	unix_to_dos_time(st.atime, &rsp->LastAccessTime, &rsp->LastAccessDate);
	unix_to_dos_time(st.mtime, &rsp->LastWriteTime, &rsp->LastWriteDate);

	rsp->FileDataSize =
		cpu_to_le32((u32)min_t(loff_t, st.size, 0xFFFFFFFF));
	rsp->FileAllocationSize =
		cpu_to_le32((u32)min_t(loff_t, st.blocks << 9, 0xFFFFFFFF));
	rsp->FileAttributes = cpu_to_le16(smb_get_dos_attr(&st));
	rsp->ByteCount = 0;

	inc_rfc1001_len(&rsp->hdr, 11 * 2 + 2);
	return 0;
}

/**
 * smb_set_information2() - SET_INFORMATION2 handler
 * @work:	smb work containing request
 *
 * Sets file timestamps using the SMB1 DOS date/time format.
 *
 * Return:	0 on success, otherwise error
 */
int smb_set_information2(struct ksmbd_work *work)
{
	...
	fp = ksmbd_lookup_fd_fast(work, req->Fid);
	if (!fp) {
		pr_err("SET_INFORMATION2: invalid Fid %u\n", req->Fid);
		rsp->hdr.Status.CifsError = STATUS_INVALID_HANDLE;
		return -ENOENT;
	}
	...
	/* Only set time fields that are non-zero */
	if (req->LastWriteDate || req->LastWriteTime) {
		time64_t t = dos_date_time_to_unix(req->LastWriteDate,
						   req->LastWriteTime);
		if (t) {
			attrs.ia_mtime.tv_sec = t;
			attrs.ia_mtime.tv_nsec = 0;
			attrs.ia_valid |= ATTR_MTIME | ATTR_MTIME_SET;
		}
	}
	...
	if (req->LastAccessDate || req->LastAccessTime) {
		time64_t t = dos_date_time_to_unix(req->LastAccessDate,
						   req->LastAccessTime);
		if (t) {
			attrs.ia_atime.tv_sec = t;
	...
	if (attrs.ia_valid) {
		rc = ksmbd_vfs_setattr(work, NULL, (u64)req->Fid, &attrs);
		if (rc) {
			rsp->hdr.Status.CifsError = STATUS_ACCESS_DENIED;
			return rc;
		}
...
int smb1_check_sign_req(struct ksmbd_work *work)
{
	...
	char signature[20];
	struct kvec iov[1];
	__u32 recv_seq, expected_seq;

	/*
	 * SMB1-09: Replay attack prevention via sequence number validation.
	 *
	 * MS-CIFS §3.3.5.2.1: The SecuritySignature field when signing is
	 * enabled contains a monotonically increasing sequence number.
	 * Requests use even sequence numbers; the server increments by 2 per
	 * exchange.  Validate that the received sequence number advances
	 * monotonically to reject replayed PDUs.
	 *
	 * Extract the sequence number from the wire BEFORE zeroing it for
	 * MAC computation.  The field is little-endian 32-bit at bytes 0-3
	 * of SecuritySignature.
	 */
	recv_seq = le32_to_cpu(*(__le32 *)rcv_hdr1->Signature.SecuritySignature);

	/* Expected seq is the next even number after current counter */
	expected_seq = work->sess->sequence_number + 1;
	...
	/*
	 * Validate sequence number monotonicity after MAC verification.
	 * Only reject if the received number is strictly less than expected
	 * (i.e., a replay of an older PDU).  Allow the first PDU (seq=1)
	 * and forward-advancing sequences.
	 */
	if (recv_seq < expected_seq && expected_seq > 1) {
		pr_warn_ratelimited("SMB1 signing: seq replay detected (got %u, expected >= %u)\n",
...
/*
 * NT_TRANSACT (0xA0) implementation
 *
 * smb_build_ntransact_rsp() fills the smb_com_ntransact_rsp header and
 * sets ByteCount.  Parameters are placed at a fixed offset (ParameterOffset
 * = 68 from Protocol[0]) and data follows at ALIGN(ParameterOffset +
 * param_len, 4).
 * Returns a pointer to the data area so callers can write payloads there.
 *
 * Layout from Protocol[0]:
				     ...
				     unsigned int data_len)
{
	...
	/* Offsets are from Protocol[0] (= 4 bytes into the transport frame) */
	...
	rsp->DataOffset           = cpu_to_le32(data_off);
	rsp->DataDisplacement     = 0;
	rsp->SetupCount           = 0;
	rsp->ByteCount = cpu_to_le16(1 /* pad */
				     + param_len
				     + (data_off - (param_off + param_len))
				     + data_len);

	inc_rfc1001_len(&rsp->hdr, 18 * 2 + le16_to_cpu(rsp->ByteCount));
...
/*
 * smb_ntransact_fill_uid_gid() - populate smb_fattr uid/gid/mode/acl
 *
 * This is equivalent to ksmbd_acls_fattr() from smb2pdu.c but local to
 * smb1pdu.c because that function is declared static there.
 */
static void smb_ntransact_fill_fattr(struct smb_fattr *fattr,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
...
/* T6 — NT_TRANSACT_CREATE (subcommand 0x01)
 *
 * The NT_TRANSACT_CREATE parameter block is 57 bytes and carries the same
 * fields as NT_CREATE_ANDX plus a SecurityDescriptorLength, EALength and
 * a NameLength.  The actual filename follows in the data block.
 *
 * For simplicity we delegate to smb_nt_create_andx() by synthesising a
 * minimal NT_CREATE_ANDX request header that points at the same filename.
 * If a non-zero SecurityDescriptorLength is present we apply the SD after
 * the file has been opened.
 *
 * MS-SMB §2.2.4.79.1
 */
static int smb_nt_transact_create(struct ksmbd_work *work,
				  struct smb_com_ntransact_req *req)
{
	...
	char *params;
	char *data;
	u32 param_count, data_count;
	u32 sd_len, ea_len, name_len;
	__le32 desired_access, file_attributes, share_access;
	__le32 create_disposition, create_options;
	__le64 alloc_size;
	char *name;
	...
	param_count = le32_to_cpu(req->ParameterCount);
	data_count  = le32_to_cpu(req->DataCount);

	/* Minimum NT_TRANSACT_CREATE parameter block: 57 bytes */
	if (param_count < 57) {
		rsp->hdr.Status.CifsError = STATUS_INVALID_PARAMETER;
		return -EINVAL;
	}
	...
	data   = (char *)req + le32_to_cpu(req->DataOffset);

	/*
	 * NT_TRANSACT_CREATE parameter layout (MS-SMB §2.2.4.79.1):
	 *  0: Flags        (4)
	 *  4: RootDirectoryFID (4)
	 *  8: DesiredAccess (4)
	 * 12: AllocationSize (8)
	 * 20: FileAttributes (4)
	 * 24: ShareAccess (4)
	 * 28: CreateDisposition (4)
	 * 32: CreateOptions (4)
	 * 36: SecurityDescriptorLength (4)
	 * 40: EALength (4)
	 * 44: NameLength (4)
	 * 48: ImpersonationLevel (4)
	 * 52: SecurityFlags (1)
	 * 53: (pad 4 bytes to 57)
	 */
	desired_access      = *(__le32 *)(params + 8);
	alloc_size          = *(__le64 *)(params + 12);
	file_attributes     = *(__le32 *)(params + 20);
	share_access        = *(__le32 *)(params + 24);
	create_disposition  = *(__le32 *)(params + 28);
	create_options      = *(__le32 *)(params + 32);
	sd_len              = le32_to_cpu(*(__le32 *)(params + 36));
	ea_len              = le32_to_cpu(*(__le32 *)(params + 40));
	name_len            = le32_to_cpu(*(__le32 *)(params + 44));

	ksmbd_debug(SMB,
		    "NT_TRANSACT_CREATE: da=0x%x attr=0x%x disp=0x%x opts=0x%x sd_len=%u name_len=%u\n",
		    le32_to_cpu(desired_access), le32_to_cpu(file_attributes),
		    le32_to_cpu(create_disposition), le32_to_cpu(create_options),
		    sd_len, name_len);
	...
	/*
	 * The filename is in the data block, Unicode (UTF-16LE).
	 * sd_len bytes of SD precede the filename when ea_len > 0 or
	 * sd_len > 0; for simplicity we skip the SD and EA prefix here
	 * and find the name at offset sd_len + ea_len in the data block.
	 */
	...
	name = smb_strndup_from_utf16(data + sd_len + ea_len,
				      name_len, true,
				      work->conn->local_nls);
	if (IS_ERR(name)) {
		rsp->hdr.Status.CifsError = STATUS_OBJECT_NAME_INVALID;
				 ...
				 struct smb_com_ntransact_req *req)
{
	...
	params = (char *)req + le32_to_cpu(req->ParameterOffset);
	function_code = le32_to_cpu(*(__le32 *)params);
	fid           = le16_to_cpu(*(__le16 *)(params + 4));

	ksmbd_debug(SMB,
		    "NT_TRANSACT_IOCTL: function=0x%08x fid=%u is_fsctl=%u flags=%u\n",
		    function_code, fid,
...
static int smb_nt_transact_set_security(struct ksmbd_work *work,
					struct smb_com_ntransact_req *req)
{
	struct smb_com_ntransact_rsp *rsp = work->response_buf;
	struct ksmbd_file *fp;
	...
	/* Parameters: FID(2) + Reserved(2) + SecurityInformation(4) */
	{
		char *params = (char *)req + le32_to_cpu(req->ParameterOffset);

		fid = le16_to_cpu(*(__le16 *)params);
	}
	...
	data_off = le32_to_cpu(req->DataOffset);
	data_len = le32_to_cpu(req->DataCount);
	pntsd = (struct smb_ntsd *)((char *)req + data_off);

	rc = set_info_sec(work->conn, work->tcon, &fp->filp->f_path,
			  pntsd, data_len, false, true);
	ksmbd_fd_put(work, fp);
...
/* T2b — NT_TRANSACT_NOTIFY_CHANGE (subcommand 0x04)
 *
 * Parameter block layout (MS-SMB §2.2.4.61.1):
 *   0: CompletionFilter (4) — bitmask of FILE_NOTIFY_CHANGE_* bits
 *   4: Fid              (2)
					 ...
					 struct smb_com_ntransact_req *req)
{
	...
	params           = (char *)req + le32_to_cpu(req->ParameterOffset);
	completion_filter = le32_to_cpu(*(__le32 *)params);
	fid              = le16_to_cpu(*(__le16 *)(params + 4));
	watch_tree       = ((__u8 *)params)[6];

	ksmbd_debug(SMB,
		    "NT_TRANSACT_NOTIFY_CHANGE: fid=%u filter=0x%x watch_tree=%u\n",
		    fid, completion_filter, watch_tree);
...
/* T3 — NT_TRANSACT_RENAME (subcommand 0x05)
 *
 * Parameter block layout (MS-SMB §2.2.4.79.5):
 *   0: Fid   (2)
 *   2: Flags (2) — 0x01 = replace-if-exists
 * Data block: new filename (Unicode UTF-16LE)
 */
static int smb_nt_transact_rename(struct ksmbd_work *work,
				  struct smb_com_ntransact_req *req)
{
	struct smb_com_ntransact_rsp *rsp = work->response_buf;
	u32 param_count = le32_to_cpu(req->ParameterCount);
	...
	params = (char *)req + le32_to_cpu(req->ParameterOffset);
	fid    = le16_to_cpu(*(__le16 *)params);
	/* flags = le16_to_cpu(*(__le16 *)(params + 2)); — reserved for future use */

	ksmbd_debug(SMB, "NT_TRANSACT_RENAME: fid=%u\n", fid);
	...
	/*
	 * SMB1-25: NT_TRANSACT_RENAME name length validation.
	 *
	 * Validate that the data block (containing the new filename) is
	 * fully contained within the received request buffer.
	 */
	{
		u32 req_buf_len = get_rfc1002_len(req);
		u32 data_off    = le32_to_cpu(req->DataOffset);

		if (data_count == 0 ||
		    data_off > req_buf_len ||
		    data_count > req_buf_len - data_off) {
			rsp->hdr.Status.CifsError = STATUS_INVALID_PARAMETER;
			ksmbd_fd_put(work, fp);
			return -EINVAL;
		}
	...
	/* New filename is in the data block as Unicode UTF-16LE */
	newname = smb_strndup_from_utf16(
			(char *)req + le32_to_cpu(req->DataOffset),
			data_count, true, work->conn->local_nls);
	if (IS_ERR(newname)) {
		rsp->hdr.Status.CifsError = STATUS_OBJECT_NAME_INVALID;
		ksmbd_fd_put(work, fp);
		return PTR_ERR(newname);
	...
	ksmbd_debug(SMB, "NT_TRANSACT_RENAME: new name '%s'\n", newname);
...
/* T4 — NT_TRANSACT_QUERY_SECURITY_DESC (subcommand 0x06) */
static int smb_nt_transact_query_security(struct ksmbd_work *work,
					  struct smb_com_ntransact_req *req)
{
	struct smb_com_ntransact_rsp *rsp = work->response_buf;
	struct ksmbd_file *fp;
	struct smb_ntsd *ppntsd = NULL;
	struct smb_ntsd *out_pntsd;
	struct smb_fattr fattr;
	struct inode *inode;
	__u32 secdesclen = 0;
	unsigned int max_data;
	unsigned int secinfo;
	int ppntsd_size = 0;
	int rc;
	u16 fid;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
	...
	/* Parameters: FID(2) + Reserved(2) + SecurityInformation(4) */
	{
		...
		fid     = le16_to_cpu(*(__le16 *)params);
		secinfo = le32_to_cpu(*(__le32 *)(params + 4));
	}
	...
	max_data = le32_to_cpu(req->MaxDataCount);
	...
	inode = file_inode(fp->filp);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
	smb_ntransact_fill_fattr(&fattr, idmap, inode);
#else
	smb_ntransact_fill_fattr(&fattr, user_ns, inode);
#endif
	...
	/* Build response with room for max_data bytes of SD */
	out_pntsd = smb_build_ntransact_rsp(work, 0, max_data);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
	rc = build_sec_desc(idmap, out_pntsd, ppntsd, ppntsd_size,
#else
	rc = build_sec_desc(user_ns, out_pntsd, ppntsd, ppntsd_size,
#endif
			    secinfo, &secdesclen, &fattr, max_data);
	...
	if (secdesclen > max_data) {
		/* Return required size in 4-byte parameter area */
		char *params;

		smb_build_ntransact_rsp(work, 4, 0);
		params = (char *)rsp + 4 + le32_to_cpu(rsp->ParameterOffset);
		*(__le32 *)params = cpu_to_le32(secdesclen);
		rsp->hdr.Status.CifsError = STATUS_BUFFER_TOO_SMALL;
		return 0;
	}

	/* Rebuild with actual SD size */
	smb_build_ntransact_rsp(work, 0, secdesclen);
	return 0;
}

/* T4/T5 — NT_TRANSACT_GET_USER_QUOTA (0x07) / SET_USER_QUOTA (0x08)
 *
 * User quota operations are not supported by ksmbd.  Return
 * STATUS_NOT_SUPPORTED so clients (Windows Explorer, net.exe) can gracefully
 * disable quota display.
 */
static int smb_nt_transact_get_user_quota(struct ksmbd_work *work,
					  struct smb_com_ntransact_req *req)
{
	struct smb_com_ntransact_rsp *rsp = work->response_buf;
...
/**
 * smb_nt_transact() - handler for SMB_COM_NT_TRANSACT (0xA0)
 * @work:	smb work containing NT_TRANSACT request buffer
 *
 * Dispatches to NT_TRANSACT sub-command handlers.
 *
 * Return:	0 on success, otherwise error
 */
int smb_nt_transact(struct ksmbd_work *work)
{
	...
	switch (function) {
	case NT_TRANSACT_CREATE:
		return smb_nt_transact_create(work, req);
	case NT_TRANSACT_IOCTL:
		return smb_nt_transact_ioctl(work, req);
	case NT_TRANSACT_SET_SECURITY_DESC:
		return smb_nt_transact_set_security(work, req);
	case NT_TRANSACT_NOTIFY_CHANGE:
		return smb_nt_transact_notify_change(work, req);
	case NT_TRANSACT_RENAME:
		return smb_nt_transact_rename(work, req);
	case NT_TRANSACT_QUERY_SECURITY_DESC:
		return smb_nt_transact_query_security(work, req);
	case NT_TRANSACT_GET_USER_QUOTA:
		return smb_nt_transact_get_user_quota(work, req);
	case NT_TRANSACT_SET_USER_QUOTA:
		return smb_nt_transact_set_user_quota(work, req);
	default:
		ksmbd_debug(SMB, "NT_TRANSACT: unsupported function 0x%x\n",
			    function);
		rsp->hdr.Status.CifsError = STATUS_NOT_SUPPORTED;
		return -EOPNOTSUPP;
	}
