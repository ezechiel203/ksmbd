...
static struct smb_version_values smb1_server_values = {
	...
	/* SMB1-10: MaxBufferSize is 16-bit in the SMB1 NEGOTIATE response.
	 * Per MS-SMB §2.2.4.52.2 the field is 32-bit but clients treat it
	 * as the upper bound for a single non-large-writex request.
	 * Cap to 65535 (USHRT_MAX).  CAP_LARGE_WRITEX allows larger I/Os.
	 */
...
static struct smb_version_cmds smb1_server_cmds[256] = {
	...
	[SMB_COM_FIND]			= { .proc = smb_find, },
	[SMB_COM_FIND_UNIQUE]		= { .proc = smb_find, },
	/* A.2: QUERY_INFORMATION2 and SET_INFORMATION2 */
