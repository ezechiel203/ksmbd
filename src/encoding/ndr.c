...
{
	char *data;
	size_t needed, new_sz;

	/*
	 * Compute the minimum size needed: offset + sz.
	 * Then add 1024 bytes of headroom to reduce repeated reallocs.
	 * Track both to ensure n->length accurately reflects the
	 * allocated buffer size.
	 */
	if (check_add_overflow((size_t)n->offset, sz, &needed) ||
	    check_add_overflow(needed, (size_t)1024, &new_sz))
		return -EOVERFLOW;
...
static int ndr_read_string(struct ndr *n, void *value, size_t sz)
{
	...
	if (value) {
		...
		/* Ensure null termination regardless of source data */
		((char *)value)[len] = '\0';
	}
	len++;
	n->offset += len;
...
int ndr_encode_dos_attr(struct ndr *n, struct xattr_dos_attrib *da)
{
	...
	if (da->version == 3) {
		...
		/*
		 * Version 4 is a simplified format: only itime and
		 * create_time are stored. change_time is intentionally
		 * omitted — it is derived from the filesystem's ctime
		 * rather than persisted in the xattr.
		 */
