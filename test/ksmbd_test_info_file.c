...
{
	/* FileStatLxInformation extends FileStatInformation with Lx fields */
	/* Since the handler shares the same buffer validation, test acceptance */
	u8 buf[56];
	unsigned int out_len;
	int ret;
...
static void test_file_stat_lx_xattr_fallback(struct kunit *test)
{
	/* No xattr: falls back to kstat -- same code path */
	u8 buf[56];
	unsigned int out_len;
	int ret;
...
static struct kunit_case ksmbd_info_file_test_cases[] = {
	...
	KUNIT_CASE(test_file_stat_lx_xattr_fallback),
