...
static unsigned int test_calc_credit_charge(u64 req_len, u64 resp_len)
{
	...
	unsigned int charge = DIV_ROUND_UP(max_len, TEST_SMB2_MAX_BUFFER_SIZE);

	/* MS-SMB2 §3.1.5.2: minimum credit charge is always 1 */
	return max_t(unsigned int, charge, 1);
}
...
/*
 * test_credit_charge_zero - zero-length request should require 1 credit
 *
 * MS-SMB2 §3.1.5.2: credit charge formula always yields minimum 1.
 * Even a zero-length payload consumes 1 credit.
 */
