...
/* Stricter-than-spec: ksmbd validates ChannelSequence on FLUSH/LOCK; */
/* spec only mandates WRITE/SET_INFO/IOCTL (MS-SMB2 §3.3.5.2.10)    */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Test: ChannelSequence with SMB 2.1 — NOT validated (3.x only)     */
/* MS-SMB2 §3.3.5.2.10: ChannelSequence only applies to 3.x family. */
/* SMB 2.1 (0x0210) < SMB30 (0x0300), so validation is bypassed.    */
/* ------------------------------------------------------------------ */
static void test_channel_seq_smb21_validated(struct kunit *test)
{
	...
	/* SMB 2.1 < 3.0: stale sequence should be accepted (bypassed) */
	KUNIT_EXPECT_EQ(test, 0,
