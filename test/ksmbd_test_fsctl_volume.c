...
{
	/*
	 * FSCTL_IS_VOLUME_DIRTY returns a __le32 flags field.
	 * ksmbd always returns 0 (volume not dirty), with output
	 * size equal to sizeof(__le32) = 4 bytes.
	 */
	__le32 volume_dirty = cpu_to_le32(0);
	unsigned int out_len = sizeof(__le32);

	KUNIT_EXPECT_EQ(test, le32_to_cpu(volume_dirty), (u32)0);
	KUNIT_EXPECT_EQ(test, out_len, 4U);
}

static void test_volume_dirty_buffer_too_small(struct kunit *test)
{
	/*
	 * Output buffer must hold at least 4 bytes (__le32).
	 * A 3-byte buffer is too small.
	 */
	unsigned int max_out_len = 3;

	KUNIT_EXPECT_LT(test, max_out_len, 4U);
}
...
{
	/*
	 * Validate that the struct size constant matches the actual
	 * packed struct layout (all fields sum to the expected total).
	 */
	KUNIT_EXPECT_EQ(test,
			(unsigned int)sizeof(struct test_ntfs_volume_data_rsp),
			/* 5x__le64 + 4x__le32 + 2x__le64 + 3x__le64 = 96 */
			96U);
}

static void test_ntfs_volume_data_no_share(struct kunit *test)
{
	kunit_skip(test, "TODO: needs production function export");
}
...
{
	/* StartingVcnInput is a single __le64, so minimum is 8 bytes */
	KUNIT_EXPECT_EQ(test,
			(unsigned int)sizeof(struct test_starting_vcn_input),
			8U);
}

static void test_retrieval_pointers_buffer_too_small(struct kunit *test)
{
	/* Header: ExtentCount(4) + Reserved(4) + StartingVcn(8) = 16 bytes */
	KUNIT_EXPECT_EQ(test,
			(unsigned int)sizeof(struct test_retrieval_pointers_hdr),
			16U);
}
