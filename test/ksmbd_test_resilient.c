...
static void test_resilient_timeout_over_max_rejected(struct kunit *test)
{
	struct test_network_resiliency_request req = {};
	unsigned int timeout;
	int ret = 0;
	...
	/*
	 * MS-SMB2 section 3.3.5.15.9: if the requested timeout exceeds the
	 * maximum, the server MUST fail with STATUS_INVALID_PARAMETER.
	 */
...
static void test_resilient_timeout_one_over_max(struct kunit *test)
{
	...
	/*
	 * MS-SMB2 section 3.3.5.15.9: timeout > max MUST fail.
	 */
	if (timeout > TEST_MAX_RESILIENT_TIMEOUT_MS)
		ret = -EINVAL;
...
static void test_resilient_timeout_uint_max(struct kunit *test)
{
	...
	/*
	 * MS-SMB2 section 3.3.5.15.9: UINT_MAX far exceeds the
	 * maximum; server MUST fail with STATUS_INVALID_PARAMETER.
	 */
	if (timeout > TEST_MAX_RESILIENT_TIMEOUT_MS)
		ret = -EINVAL;
...
static struct kunit_case ksmbd_resilient_test_cases[] = {
	...
	KUNIT_CASE(test_resilient_timeout_over_max_rejected),
