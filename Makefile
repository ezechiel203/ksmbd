...
DEFAULT_KVER := $(shell if [ -d /usr/lib/modules ]; then \
	latest=$$(ls -1 /usr/lib/modules 2>/dev/null | sort -V | tail -n1); \
	if [ -n "$$latest" ]; then printf '%s' "$$latest"; else uname -r; fi; \
else \
	uname -r; \
fi)
KVER ?= $(DEFAULT_KVER)
KDIR ?= /usr/lib/modules/$(KVER)/build
MDIR ?= /usr/lib/modules/$(KVER)
PWD := $(shell pwd)
MODULE_NAME := ksmbd
