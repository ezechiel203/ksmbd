...
static int smb1_oplock_break_noti(struct oplock_info *opinfo)
{
	...
	/* H-11: Pin conn refcount against concurrent teardown */
	if (!conn || !atomic_inc_not_zero(&conn->refcnt))
		return -ENOENT;
...
static int smb2_oplock_break_noti(struct oplock_info *opinfo)
{
	...
	/*
	 * H-11: Pin conn refcount before using it. opinfo->conn can
	 * be set to NULL by the durable handle disconnect path in
	 * vfs_cache.c while opinfo is still alive.
	 */
	if (!conn || !atomic_inc_not_zero(&conn->refcnt))
		return -ENOENT;
...
void destroy_lease_table(struct ksmbd_conn *conn)
{
	...
	/*
	 * H-10: write_lock(&lease_list_lock) is a spinlock context.
	 * Only list manipulation and kfree() occur under it (no
	 * GFP_KERNEL allocations), which is safe.
	 */
	write_lock(&lease_list_lock);
...
static int add_lease_global_list(struct oplock_info *opinfo)
{
	...
	/*
	 * H-10: lease_list_lock is a rwlock (spinlock-based).
	 * No GFP_KERNEL allocations occur while it is held:
	 * - The read_lock section below only does list traversal
	 *   and lease_add_list (which uses lb->lb_lock spinlock).
	 * - The kmalloc below happens after read_unlock.
	 * - lb_add() only holds write_lock for list_add.
	 */
	read_lock(&lease_list_lock);
	list_for_each_entry(lb, &lease_table_list, l_entry) {
	...
	/* Allocation is safely outside the lock section */
...
/**
 * smb_break_dir_lease() - break directory leases on a specific directory
 * @dir_dentry:	dentry of the directory whose leases should be broken
 *
 * Break all leases held on the given directory.
 */
void smb_break_dir_lease(struct dentry *dir_dentry)
{
	struct oplock_info *opinfo;
	...
	p_ci = ksmbd_inode_lookup_lock(dir_dentry);
	if (!p_ci)
		return;
...
/**
 * smb_break_parent_dir_lease() - break parent directory leases on child modification
 * @fp:		ksmbd file pointer of the child file being modified
 *
 * When a child object in a directory is modified (setinfo, rename, unlink,
 * hardlink), any directory leases held on the parent directory must be broken
 * to notify the client that the directory contents have changed.
 */
void smb_break_parent_dir_lease(struct ksmbd_file *fp)
{
	if (!fp->filp || !fp->filp->f_path.dentry)
		return;

	smb_break_dir_lease(fp->filp->f_path.dentry->d_parent);
}
