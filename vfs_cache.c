...
static void __ksmbd_inode_close(struct ksmbd_file *fp)
{
	...
	if (atomic_dec_and_test(&ci->m_count)) {
		...
		if (do_unlink) {
			/* Break parent directory leases before unlinking */
...
static void __ksmbd_close_fd(struct ksmbd_file_table *ft, struct ksmbd_file *fp)
{
	...
	/* H-01: remove on-disk persistent handle state */
	if (fp->is_persistent)
		ksmbd_ph_delete(fp);
...
/**
 * ksmbd_close_prev_app_instance() - Close prior opens with same app instance
 * @fp: the newly opened file (already in the inode fp_list)
 *
 * Walk the inode's m_fp_list looking for an existing open with the same
 * app_instance_id.  When APP_INSTANCE_VERSION was also supplied, the old
 * handle is only closed when the new (high, low) version tuple is strictly
 * greater; otherwise the new open fails (caller must handle -EPERM).
 *
 * Must be called AFTER @fp has been added to ci->m_fp_list and AFTER
 * has_app_instance_id / version fields are set on @fp.
 *
 * Returns:
 *   0      – previous instance closed (or no previous instance found)
 *  -EPERM  – existing instance has equal-or-newer version; new open loses
 */
int ksmbd_close_prev_app_instance(struct ksmbd_file *fp)
{
	struct ksmbd_inode *ci = fp->f_ci;
	struct ksmbd_file *prev_fp;
	struct ksmbd_file_table *found_ft = NULL;
	u64 found_volatile_id = KSMBD_NO_FID;
	int rc = 0;
	...
	list_for_each_entry(prev_fp, &ci->m_fp_list, node) {
		...
		if (memcmp(prev_fp->app_instance_id,
			   fp->app_instance_id, 16))
			continue;

		/*
		 * Same AppInstanceId on same inode.
		 * If a version context was supplied, only close the prior
		 * open when the new version is strictly higher.
		 */
		if (fp->has_app_instance_version) {
			u64 pv_high = prev_fp->app_instance_version;
			u64 pv_low  = prev_fp->app_instance_version_low;
			u64 nv_high = fp->app_instance_version;
			u64 nv_low  = fp->app_instance_version_low;

			if (pv_high > nv_high ||
			    (pv_high == nv_high && pv_low >= nv_low)) {
				/* Existing version >= new version: new open loses */
				rc = -EPERM;
				break;
			}
		}
		...
		/*
		 * Capture volatile_id and owning file_table under m_lock.
		 * The ft pointer is stable for as long as the file handle
		 * exists (session lifetime >= open handle lifetime).
		 */
		found_volatile_id = prev_fp->volatile_id;
		found_ft = prev_fp->ft;
		break;
	}
	up_read(&ci->m_lock);
	...
	if (!has_file_id(found_volatile_id) || !found_ft)
		return 0;

	ksmbd_debug(SMB,
		    "APP_INSTANCE: closing previous open fid=%llu\n",
		    found_volatile_id);

	/*
	 * Remove the previous fp from its owning file table's idr.
	 * Pattern mirrors __close_file_table_ids(): mark volatile_id
	 * invalid under the lock, then call __ksmbd_close_fd(ft, fp)
	 * where __ksmbd_remove_fd() will short-circuit on KSMBD_NO_FID.
	 */
	write_lock(&found_ft->lock);
	prev_fp = idr_find(found_ft->idr, found_volatile_id);
	if (prev_fp) {
		set_close_state_blocked_works(prev_fp);
		if (prev_fp->f_state != FP_INITED ||
		    !atomic_dec_and_test(&prev_fp->refcount))
			prev_fp = NULL;
		else {
			prev_fp->f_state = FP_CLOSED;
			idr_remove(found_ft->idr, found_volatile_id);
			prev_fp->volatile_id = KSMBD_NO_FID;
		}
	}
	write_unlock(&found_ft->lock);

	if (!prev_fp)
		return 0;

	/* Remove from inode fp_list */
	down_write(&ci->m_lock);
	list_del_init(&prev_fp->node);
	up_write(&ci->m_lock);

	__ksmbd_close_fd(found_ft, prev_fp);
	return 0;
}
