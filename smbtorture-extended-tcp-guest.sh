#!/bin/bash
# Extended SMB2 smbtorture suite over direct TCP.
# Covers locking, notify, streams, ioctl, rename, rw, setinfo, dosmode,
# timestamps, session, delete-on-close, sharemode, compound_async,
# oplock, lease, durable-open, durable-v2-regressions.
#
# Prerequisites: vm-guest-prepare.sh must have run with leases + durable
# handles enabled (smb2 leases = yes, durable handles = yes in ksmbd.conf).
...
smbtorture "$UNC" --list smb2.session 2>/dev/null \
...
tests=(
  ...
  # ── client caching (oplock / lease / dir-lease) ───────────
  smb2.oplock
  smb2.lease
  smb2.dirlease
...
for t in "${tests[@]}"; do
  # Connectivity pre-check: if the server is down, report INFRA_FAIL
  # instead of letting the test fail with misleading error output.
  ...
  tmp_out="/tmp/smbtorture-ext-${t//./_}.out"
  echo "=== ${t} ==="
...
echo ""
echo "TCP_EXTENDED PASS=${pass} FAIL=${fail} SKIP=${skip} INFRA=${infra} KNOWNFAIL=${knownfail}"
[ "$fail" -eq 0 ]
